...
class MemorialCalculoFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # --- LINHA CORRIGIDA: A definição de self.col_widths foi adicionada de volta ---
    ...
    def _construir_bloco_tabelas(self, parent, vars_storage, tables_storage, refs_storage, tipo_bloco):
        ...
        if tipo_bloco == "original":
            self.frame_tabela_header = header
        else:
            self.frame_tabela_header_rep = header
        ...
        for i in range(1, 4):
            ...
            if tipo_bloco == "original":
                for var_name in group_vars:
                    ...
                    if db_col_name not in self.colunas_db:
                        self.colunas_db.append(db_col_name)
        
        self.criar_tabela_grupo4(parent, tables_storage)
        
        if tipo_bloco == "original" and "observacoes" not in self.colunas_db:
    ...
    def _alinhar_coluna_critica_replicada(self):
        ...
        if not self.ref_cells_rep: return
        reference_width = self.ref_cells_rep[0].winfo_width()
        if reference_width > 1:
            if self.frame_tabela_header_rep:
                self.frame_tabela_header_rep.grid_columnconfigure(3, minsize=reference_width)
            for table in self.tabelas_grupo_frames_rep:
