...
from visualizacao_frame import VisualizacaoFrame # <-- 1. IMPORTAR O NOVO MÓDULO
...
class App(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # <-- 2. ADICIONAR O NOVO FRAME À LISTA DE PÁGINAS
        pages_to_load = (MenuFrame, MemorialCalculoFrame, AjustesFrame, VisualizacaoFrame)
    ...
    def show_frame(self, page_name):
        ...
        # Adiciona uma verificação para garantir que a página existe
        if page_name in self.frames:
            frame = self.frames[page_name]
            frame.tkraise()
        else:
            messagebox.showwarning("Módulo não encontrado",
                                   f"O módulo '{page_name}' não foi carregado no sistema.")
    ...
    def show_login_popup(self, target_page_name):
        ...
        def on_login_success():
            ...
            if target_page_name in self.frames:
                # Agora que VisualizacaoFrame existe, este código irá exibi-lo
                self.show_frame(target_page_name)
            else:
                # Este bloco de código não será mais alcançado para VisualizacaoFrame
