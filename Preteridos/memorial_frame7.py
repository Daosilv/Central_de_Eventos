...
class MemorialCalculoFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # --- MUDANÇA: Estilo global para garantir fundo branco em todos os widgets ---
        style = ttk.Style()
        style.theme_use('clam')
        
        # Configuração global para todos os widgets ttk
        style.configure('.', background='white', foreground='black')
        
        # Configurações específicas para manter o fundo do TCombobox branco na seleção
        style.map('TCombobox',
                  fieldbackground=[('readonly', 'white')],
                  selectbackground=[('readonly', 'white')],
                  selectforeground=[('readonly', 'black')])
        
        # Estilo para remover a seta do Combobox
    ...
    def criar_layout_principal(self):
        # --- MUDANÇA: Adicionado bg='white' aos containers principais ---
        ...
        self.canvas.create_window((0, 0), window=self.content_holder_frame, anchor="nw")
        ...
        concluidos_container = tk.LabelFrame(sidebar_frame, text="Concluídos", padx=5, pady=5, bg='white'); concluidos_container.pack(fill='both', expand=True, pady=(5, 0))
        frame_busca_concluidos = tk.Frame(concluidos_container, bg='white'); frame_busca_concluidos.pack(fill='x')
        tk.Label(frame_busca_concluidos, text="Buscar:", bg='white').pack(side='left')
        tk.Entry(frame_busca_concluidos, textvariable=self.busca_concluidos_var).pack(side='left', fill='x', expand=True, padx=5)
        frame_concluidos_list = tk.Frame(concluidos_container, bg='white'); frame_concluidos_list.pack(fill='both', expand=True, pady=(5,0))
        scrollbar_concluidos = ttk.Scrollbar(frame_concluidos_list); scrollbar_concluidos.pack(side='right', fill='y')
        self.listbox_concluidos = tk.Listbox(frame_concluidos_list, yscrollcommand=scrollbar_concluidos.set); self.listbox_concluidos.pack(side='left', fill='both', expand=True)
    ...
    def criar_layout_formulario(self):
        ...
        self.frame_tabela_header = tk.Frame(wrap_tabelas, bd=1, relief="solid", bg='white')
        ...
        self.criar_tabela_grupo4(wrap_tabelas)
    ...
    def criar_tabela_grupo4(self, parent):
        ...
        for i, minsize in enumerate(self.col_widths): 
            frame_tabela4.grid_columnconfigure(i, minsize=minsize)
        ...
        self.tabelas_grupo_frames.append(frame_tabela4)
    ...
    def criar_tabela_grupo(self, parent, group_name):
        ...
        cell_pickup_fase = tk.Frame(frame_tabela, relief="solid", bd=1, bg='white'); cell_pickup_fase.grid_propagate(False); cell_pickup_fase.grid(row=1, column=1, rowspan=3, sticky="nsew")
        ...
        cell_pickup_terra = tk.Frame(frame_tabela, relief="solid", bd=1, bg='white'); cell_pickup_terra.grid_propagate(False); cell_pickup_terra.grid(row=1, column=5, rowspan=3, sticky="nsew")
    ...
    def inicializar_banco(self):
        try:
            ...
            with sqlite3.connect(caminho_bd_ativo) as conexao:
                ...
                try: cursor.execute("ALTER TABLE ajustes ADD COLUMN observacao TEXT")
    ...
    def atualizar_lista_local(self):
        ...
        try:
            ...
            with sqlite3.connect(caminho_bd_ativo) as conexao:
                ...
                for eq in cursor.fetchall(): self.lista_ajustes_completa.append(eq[0])
    ...
    def reconstruir_lista_rascunhos(self, *args):
        ...
        try:
            ...
            with sqlite3.connect(caminho_bd_ativo) as conexao:
                ...
                for eq in cursor.fetchall(): self.lista_rascunhos_completa.append(eq[0])
        ...
        for eqpto in rascunhos_filtrados:
            ...
            row = tk.Frame(self.rascunhos_items_frame); row.pack(fill='x', expand=True, pady=1)
            ttk.Checkbutton(row, variable=var).pack(side='left')
            lbl = tk.Label(row, text=eqpto, anchor='w'); lbl.pack(side='left', fill='x', expand=True)
            lbl.bind("<Double-1>", lambda e, eq=eqpto: self.carregar_rascunho_pelo_nome(eq))
            tk.Button(row, text="X", fg="red", relief="flat", cursor="hand2", font=("Helvetica", 8, "bold"), command=lambda e=eqpto: self.deletar_rascunho_individual(e)).pack(side='right')
