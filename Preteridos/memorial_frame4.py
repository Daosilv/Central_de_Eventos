...
class MemorialCalculoFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # --- MODIFICADO: Novas variáveis para a busca ---
    ...
    def criar_layout_principal(self):
        ...
        # --- MODIFICADO: Rótulo e campo de busca para Ajustes Recentes ---
        lbl_local = tk.Label(sidebar_frame, text="Ajustes Recentes")
        lbl_local.pack(fill='x')

        frame_busca_ajustes = tk.Frame(sidebar_frame)
        frame_busca_ajustes.pack(fill='x', pady=(2, 0))
        lbl_busca = tk.Label(frame_busca_ajustes, text="Buscar:")
        lbl_busca.pack(side='left')
        entry_busca_ajustes = tk.Entry(frame_busca_ajustes, textvariable=self.busca_ajustes_var)
        entry_busca_ajustes.pack(side='left', fill='x', expand=True, padx=5)
        ...
        # --- Rótulo e lista de Rascunhos ---
        lbl_rascunhos = tk.Label(sidebar_frame, text="Rascunhos")
        lbl_rascunhos.pack(fill='x', pady=(10, 0))
        frame_rascunhos_list = tk.Frame(sidebar_frame)
        frame_rascunhos_list.pack(fill='both', expand=True, pady=5)
        scrollbar_rascunhos = ttk.Scrollbar(frame_rascunhos_list)
        scrollbar_rascunhos.pack(side='right', fill='y')
        self.listbox_rascunhos = tk.Listbox(frame_rascunhos_list, yscrollcommand=scrollbar_rascunhos.set)
        self.listbox_rascunhos.pack(side='left', fill='both', expand=True)
        scrollbar_rascunhos.config(command=self.listbox_rascunhos.yview)
        self.listbox_rascunhos.bind("<Double-1>", self.carregar_rascunho_selecionado)

        # --- NOVO: Espaço para Concluídos ---
        lbl_concluidos = tk.Label(sidebar_frame, text="Concluídos")
        lbl_concluidos.pack(fill='x', pady=(10, 0))
        frame_concluidos_list = tk.Frame(sidebar_frame)
        frame_concluidos_list.pack(fill='both', expand=True, pady=5)
        ...
        # Nenhuma ação de duplo-clique por enquanto
    ...
    # =====================================================================================
    # === MÉTODOS DE LÓGICA E SUPORTE ===
    # =====================================================================================

    def _filtrar_ajustes_recentes(self, *args):
        """Filtra a lista de ajustes recentes com base no campo de busca."""
        termo_busca = self.busca_ajustes_var.get().lower()
        self.listbox_local.delete(0, tk.END)
        
        if not termo_busca:
            # Se a busca estiver vazia, exibe todos os itens
            for item in self.lista_ajustes_completa:
                self.listbox_local.insert(tk.END, item)
        else:
            # Exibe apenas os itens que correspondem ao termo de busca
            for item in self.lista_ajustes_completa:
    ...
    def inicializar_banco(self):
        """Cria as tabelas 'cadastros' e 'rascunhos' se elas não existirem."""
        try:
    ...
    def _salvar_dados(self, tabela):
        """Função auxiliar para salvar dados em uma tabela específica."""
    ...
    def salvar_rascunho(self):
        """Salva o estado atual do formulário na tabela de rascunhos."""
        if self._salvar_dados('rascunhos'):
            messagebox.showinfo("Sucesso", "Rascunho salvo com sucesso!")
            self.atualizar_lista_rascunhos()

    def salvar_e_enviar(self):
        """Salva o formulário na tabela principal e limpa qualquer rascunho associado."""
    ...
    def atualizar_lista_local(self):
        """Atualiza a lista de 'Ajustes Recentes' e a popula na busca."""
    ...
    def atualizar_lista_rascunhos(self):
        self.listbox_rascunhos.delete(0, tk.END)
        try:
            if not os.path.exists(caminho_bd_ativo): return
            with sqlite3.connect(caminho_bd_ativo) as conexao:
                cursor = conexao.cursor()
                cursor.execute("SELECT DISTINCT eqpto FROM rascunhos WHERE eqpto != '' ORDER BY data_registro DESC")
                for eq in cursor.fetchall(): self.listbox_rascunhos.insert(tk.END, eq[0])
    ...
    def carregar_ficha_por_id(self, id_ficha, tabela='cadastros', janela_pai=None):
        """Carrega uma ficha específica de uma tabela específica."""
        try:
            with sqlite3.connect(caminho_bd_ativo) as conexao:
    ...
    def carregar_ficha_selecionada(self, event=None):
        """Carrega a ficha mais recente da tabela 'cadastros'."""
        ...
        try:
            ...
            else: messagebox.showinfo("Info", f"Nenhum registro encontrado para {eqpto} em 'cadastros'.")
        except sqlite3.Error as e: messagebox.showerror("Erro de Banco de Dados", f"Erro ao buscar ficha: {e}")

    def carregar_rascunho_selecionado(self, event=None):
        """Carrega o rascunho mais recente da tabela 'rascunhos'."""
        if not self.listbox_rascunhos.curselection(): return
        eqpto = self.listbox_rascunhos.get(self.listbox_rascunhos.curselection()[0])
        try:
            with sqlite3.connect(caminho_bd_ativo) as conexao:
                cursor = conexao.cursor()
            ...
            else: messagebox.showinfo("Info", f"Nenhum registro encontrado para {eqpto} em 'rascunhos'.")
    ...
    def abrir_janela_busca(self):
        ...
        def carregar_versao_selecionada():
            ...
            self.carregar_ficha_por_id(id_ficha, tabela='cadastros', janela_pai=busca_window)
