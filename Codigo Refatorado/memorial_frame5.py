...
class MemorialCalculoFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.campos_dict = { 
            ...
            "tensao_kv": tk.StringVar(), "corrente_fase_a": tk.StringVar(), 
            "corrente_fase_b": tk.StringVar(), "corrente_fase_c": tk.StringVar(),
        ...
        self.anexos_list = [] # Lista para armazenar caminhos dos arquivos anexados
    ...
    def criar_layout_formulario(self):
        ...
        manobra_frame.pack(pady=(15, 5), padx=10, anchor='w')
        ...
        frame_anexos.pack(pady=(10, 0), padx=10, anchor='w')
    ...
    def _criar_tabela_tensao_corrente(self, parent):
        ...
        for i, (label_text, var) in enumerate(campos):
            ...
            tk.Label(label_frame, text=label_text, font=FONTE_PADRAO, bg='white', width=20, anchor='center').pack(fill="both", expand=True)
    ...
    def _set_widgets_state(self, parent, new_state):
        """Ativa ou desativa (recursivamente) todos os widgets de entrada em um widget pai."""
    ...
    def _gerenciar_bloco_tabelas_replicado(self, *args):
        ...
        if condicao in ["Acess Inv", "Acess s/Inv"]:
            ...
            for grupo_key in self.vars_grupos_rep:
                for var in self.vars_grupos_rep[grupo_key].values():
                    var.set("")
    ...
    def preencher_formulario(self, dados_dict):
        ...
        # self.anexos_list = dados_dict.get("anexos", [])
    ...
    def abrir_anexos(self):
        ...
        anexos_window.transient(self.parent_window)
        anexos_window.grab_set()
        anexos_window.configure(bg='white')

        controls_frame = tk.Frame(anexos_window, bg='white')
        controls_frame.pack(pady=10, padx=10, fill='x')

        add_frame = tk.Frame(controls_frame, bg='white')
        add_frame.pack(side='left', padx=(0, 20))
        ...
        lbl_add = tk.Label(add_frame, text="Adicionar", font=FONTE_PADRAO, bg='white')
        lbl_add.pack(side='left')

        remove_frame = tk.Frame(controls_frame, bg='white')
        ...
        lbl_remove = tk.Label(remove_frame, text="Remover", font=FONTE_PADRAO, bg='white')
        lbl_remove.pack(side='left')

        list_frame = tk.Frame(anexos_window, bg='white')
        ...
        for anexo_path in self.anexos_list:
            listbox_anexos.insert(tk.END, os.path.basename(anexo_path))
