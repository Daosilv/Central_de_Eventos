...
class AjustesFrame(tk.Frame):
    ...
    def carregar_historico(self):
        ...
        if self.lista_historico_completa:
            for item in self.lista_historico_completa:
                ...
                lbl_eqpto = tk.Label(row, text=eqpto, width=8, anchor='w', font=FONTE_PEQUENA) 
    ...
    def _criar_linha_equipamento(self, parent, dados, row_index):
        ...
        # --- INÍCIO DA ALTERAÇÃO: Layout com Grid e Separadores ---

        # Define a coluna 'Comando' (índice 14) como expansível
        COMMANDO_COLUMN_INDEX = 14
        row_frame.grid_columnconfigure(COMMANDO_COLUMN_INDEX, weight=1)

        # Coluna 0: Checkbox
        check_var = tk.BooleanVar()
        ttk.Checkbutton(row_frame, variable=check_var).grid(row=0, column=0, sticky='w', padx=(5, 2))

        # Definição dos dados e suas larguras fixas
        col_data = [
            (f"Eqpto: {dados.get('eqpto', '')}", 15),
            (f"Autor: {dados.get('autor', '')}", 25),
            (f"Data: {dados.get('data', '')}", 12),
            (f"Malha: {dados.get('malha', '')}", 15),
            (f"Alimentador: {dados.get('alimentador', '')}", 15),
            (f"Fabricante: {dados.get('fabricante', '')}", 20),
            (f"Comando: {dados.get('comando', '')}", None)  # None para largura automática/expansível
        ]

        col_idx = 1
        for text, width in col_data:
            # Adiciona um separador antes de cada coluna de dados
            ttk.Separator(row_frame, orient='vertical').grid(row=0, column=col_idx, sticky='ns')
            col_idx += 1
            ...
            # Define o alinhamento: 'ew' para a coluna expansível, 'w' para as outras
            sticky_option = 'ew' if width is None else 'w'
            tk.Label(row_frame, **label_opts).grid(row=0, column=col_idx, sticky=sticky_option, padx=5)
            col_idx += 1

        # Separador final antes das ações
        ttk.Separator(row_frame, orient='vertical').grid(row=0, column=col_idx, sticky='ns')
        col_idx += 1
        
        # Frame de ações, posicionado na última coluna
        action_frame = tk.Frame(row_frame)
        action_frame.grid(row=0, column=col_idx, sticky='e', padx=(5, 0))
