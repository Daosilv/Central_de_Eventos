...
class MemorialCalculoFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.vcmd_5 = self.register(self.limitar_5_caracteres)
        self.vcmd_7_digitos = self.register(self.limitar_7_digitos)
        ...
        self.after(10, self._gerenciar_campo_acessante) # Chamada inicial para garantir estado correto
    ...
    def _criar_layout_cabecalho(self):
        ...
        self._criar_celula_cabecalho(row3_frame, "Telecontrolado:", self.campos_dict["Telecontrolado"], ttk.Combobox, {"values": ["", "sim", "não"], "state": "readonly", "font": FONTE_PADRAO, "style": 'Arrowless.TCombobox'}, 12, 10)
        self._criar_celula_cabecalho(row3_frame, "Siom:", self.campos_dict["Siom"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 4, 10)

        # Linha 4 (Nova linha para os campos adicionados/modificados)
        row4_frame = tk.Frame(frame_campos, bg='white'); row4_frame.pack(fill="x", expand=True)
        self._criar_celula_cabecalho(row4_frame, "By-Pass:", self.campos_dict["Bypass"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO, "validate": "key", "validatecommand": (self.vcmd_5, "%P")}, 6, 8)
        self._criar_celula_cabecalho(row4_frame, "Manobra Efetiva:", self.campos_dict["Manobra Efetiva"], ttk.Combobox, {"values": ["Não", "Sim"], "state": "readonly", "font": FONTE_PADRAO, "style": 'Arrowless.TCombobox'}, 14, 8)
        self._criar_celula_cabecalho(row4_frame, "Vinculado a Acessante:", self.campos_dict["Vinculado a Acessante"], ttk.Combobox, {"values": ["Não", "Sim"], "state": "readonly", "font": FONTE_PADRAO, "style": 'Arrowless.TCombobox'}, 19, 8)
        
        # --- Campo condicional para 'Eqpto Acessante' ---
        self.frame_acessante = self._criar_celula_cabecalho(row4_frame, "Eqpto Acessante:", self.campos_dict["Eqpto Acessante"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO, "validate": "key", "validatecommand": (self.vcmd_7_digitos, "%P")}, 14, 10)
    ...
    def _criar_celula_cabecalho(self, parent, label_text, var, widget_class, widget_options, label_width, entry_width):
        ...
        if widget_class == ttk.Combobox:
            widget = widget_class(widget_container, textvariable=var, **widget_options)
        else:
            widget = widget_class(widget_container, textvariable=var, **widget_options)
    ...
    def limitar_5_caracteres(self, P): return len(P) <= 5
    def limitar_7_digitos(self, P): return P.isdigit() and len(P) <= 7 or P == ""
