...
class MemorialCalculoFrame(tk.Frame):
    ...
    def criar_layout_principal(self):
        ...
        self.content_holder_frame.grid_columnconfigure(0, minsize=900)
    ...
    def _criar_layout_cabecalho(self):
        ...
        self._criar_celula_cabecalho(row1_frame, "Condição:", self.campos_dict["Condição"], ttk.Combobox, {}, 8, 20, categoria_db='condicao')

        # Linha 2
        row2_frame = tk.Frame(frame_campos, bg='white'); row2_frame.pack(fill="x", expand=True, pady=(0, 2))
        self._criar_celula_cabecalho(row2_frame, "Malha:", self.campos_dict["Malha"], ttk.Combobox, {}, 6, 10, categoria_db='malha')
        self._criar_celula_cabecalho(row2_frame, "Alimentador:", self.campos_dict["Alimentador"], ttk.Combobox, {}, 10, 17, categoria_db='alimentador')
        self._criar_celula_cabecalho(row2_frame, "Motivador:", self.campos_dict["Motivador"], ttk.Combobox, {}, 8, 8, categoria_db='motivador')
        ...
        self._criar_celula_cabecalho(row3_frame, "Mídia:", self.campos_dict["Mídia"], ttk.Combobox, {}, 6, 10, categoria_db='midia')
        self._criar_celula_cabecalho(row3_frame, "Fabricante:", self.campos_dict["Fabricante"], ttk.Combobox, {}, 10, 20, categoria_db='fabricante')
        ...
        self._criar_celula_cabecalho(row3_frame, "Telecontrolado:", self.campos_dict["Telecontrolado"], ttk.Combobox, {}, 12, 5, categoria_db='telecontrolado')
        ...
        self._criar_celula_cabecalho(row4_frame, "Manobra Efetiva:", self.campos_dict["Manobra Efetiva"], ttk.Combobox, {}, 14, 5, categoria_db='manobra_efetiva')
        self._criar_celula_cabecalho(row4_frame, "Vinculado a Acessante:", self.campos_dict["Vinculado a Acessante"], ttk.Combobox, {}, 19, 5, categoria_db='vinculado_acessante')
    ...
    def atualizar_opcoes_comboboxes(self):
        for categoria, combobox in self.comboboxes_cabecalho.items():
            opcoes = self._get_opcoes_formatadas(categoria)
    ...
    def _criar_celula_cabecalho(self, parent, label_text, var, widget_class, widget_options, label_width, entry_width, validate_cmd=None, categoria_db=None):
        ...
        if widget_class == ttk.Combobox:
            if categoria_db:
                options['values'] = [""] + self._get_opcoes_formatadas(categoria_db)
    ...
    def _criar_novas_tabelas_parametros(self, parent):
        ...
        f3 = tk.Frame(coluna_meio, bg='white')
        ...
        f5 = tk.Frame(coluna_meio, bg='white')
        ...
        frame_montante = tk.Frame(coluna_direita, bg='white')
    ...
    def _get_form_data(self):
        data = {key.lower().replace(" ", "_"): var.get() for key, var in self.campos_dict.items()}
    ...
    def atualizar_todas_as_listas(self):
        ...
        self.atualizar_opcoes_comboboxes()
    ...
    def preencher_formulario(self, dados_dict):
        ...
        for nome_campo, var in self.campos_dict.items():
            db_key = nome_campo.lower().replace(" ", "_")
            var.set(dados_dict.get(db_key, ""))
