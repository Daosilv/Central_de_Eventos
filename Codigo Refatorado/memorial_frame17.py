...
class MemorialCalculoFrame(tk.Frame):
    ...
    def _set_fixed_spacer_width(self):
        ...
        spacer_width = window_width - col1_width - col3_width - 70 
    ...
    def _criar_celula_cabecalho(self, parent, row, col, label_text, widget_class, widget_options, label_width=15, columnspan=1):
        outer = tk.Frame(parent, relief="solid", bd=1, bg='white')
        outer.grid(row=row, column=col, columnspan=columnspan, sticky="nsew")
        
        tk.Label(outer, text=f"{label_text}:", font=FONTE_PEQUENA, anchor="w", bg='white', width=label_width).pack(side="left", padx=4, pady=4)
        
        widget = widget_class(outer, **widget_options)
        widget.pack(side="left", fill="x", expand=True, padx=(0, 4))

        return widget

    def _criar_layout_cabecalho(self):
        grid_frame = tk.Frame(self.header_frame, bg='black')
        ...
        # --- LINHA 1 (AJUSTADA) ---
        self._criar_celula_cabecalho(grid_frame, 0, 0, "Autor", tk.Entry, {"textvariable": self.campos_dict["Autor"], "font": FONTE_PEQUENA, "relief": "flat"}, label_width=4, columnspan=4)
        self._criar_celula_cabecalho(grid_frame, 0, 4, "Data", tk.Entry, {"textvariable": self.campos_dict["Data"], "font": FONTE_PEQUENA, "relief": "flat"}, label_width=4, columnspan=1)
        self._criar_celula_cabecalho(grid_frame, 0, 5, "NS", tk.Entry, {"textvariable": self.campos_dict["NS"], "font": FONTE_PEQUENA, "relief": "flat"}, label_width=3, columnspan=2)
        self._criar_celula_cabecalho(grid_frame, 0, 7, "Eqpto", tk.Entry, {"textvariable": self.campos_dict["Eqpto"], "font": FONTE_PEQUENA, "relief": "flat"}, label_width=5, columnspan=2)
        ...
        cond_combo = self._criar_celula_cabecalho(grid_frame, 0, 9, "Condição", ttk.Combobox, {"textvariable": self.campos_dict["Condição"], "values": opcoes_condicao, "state": "readonly", "font": FONTE_PEQUENA, "style": 'Arrowless.TCombobox'}, label_width=8, columnspan=3)
        cond_combo.bind("<Button-1>", self.open_dropdown_on_click)

        # --- LINHA 2 ---
        opcoes_malha = ["", "Centro", "Leste", "Mantiqueira", "Norte", "Sul"]
        malha_combo = self._criar_celula_cabecalho(grid_frame, 1, 0, "Malha", ttk.Combobox, {"textvariable": self.campos_dict["Malha"], "values": opcoes_malha, "state": "readonly", "font": FONTE_PEQUENA, "style": 'Arrowless.TCombobox'}, label_width=6, columnspan=2)
        malha_combo.bind("<Button-1>", self.open_dropdown_on_click)
        
        self._criar_celula_cabecalho(grid_frame, 1, 2, "Alimentador", tk.Entry, {"textvariable": self.campos_dict["Alimentador"], "font": FONTE_PEQUENA, "relief": "flat"}, label_width=10, columnspan=2)
        
        opcoes_motivador = ["", "Instalação", "Reajuste", "Relocação"]
        motivador_combo = self._criar_celula_cabecalho(grid_frame, 1, 4, "Motivador", ttk.Combobox, {"textvariable": self.campos_dict["Motivador"], "values": opcoes_motivador, "state": "readonly", "font": FONTE_PEQUENA, "style": 'Arrowless.TCombobox'}, label_width=8, columnspan=2)
        motivador_combo.bind("<Button-1>", self.open_dropdown_on_click)
        
        self._criar_celula_cabecalho(grid_frame, 1, 6, "Coord", tk.Entry, {"textvariable": self.campos_dict["Coord"], "font": FONTE_PEQUENA, "relief": "flat"}, label_width=5, columnspan=2)
        self._criar_celula_cabecalho(grid_frame, 1, 8, "Endereço", tk.Entry, {"textvariable": self.campos_dict["Endereço"], "font": FONTE_PEQUENA, "relief": "flat"}, label_width=8, columnspan=4)

        # --- LINHA 3 ---
        opcoes_midia = ["", "Celular", "Satélite", "Rádio", "Desconhecido", "Não enc."]
        midia_combo = self._criar_celula_cabecalho(grid_frame, 2, 0, "Mídia", ttk.Combobox, {"textvariable": self.campos_dict["Mídia"], "values": opcoes_midia, "state": "readonly", "font": FONTE_PEQUENA, "style": 'Arrowless.TCombobox'}, label_width=6, columnspan=2)
        midia_combo.bind("<Button-1>", self.open_dropdown_on_click)
        ...
        fab_combo = self._criar_celula_cabecalho(grid_frame, 2, 2, "Fabricante", ttk.Combobox, {"textvariable": self.campos_dict["Fabricante"], "values": opcoes_fabricante, "state": "readonly", "font": FONTE_PEQUENA, "style": 'Arrowless.TCombobox'}, label_width=10, columnspan=3)
        fab_combo.bind("<Button-1>", self.open_dropdown_on_click)
        
        self._criar_celula_cabecalho(grid_frame, 2, 5, "Modelo", tk.Entry, {"textvariable": self.campos_dict["Modelo"], "font": FONTE_PEQUENA, "relief": "flat"}, label_width=6, columnspan=2)
        self._criar_celula_cabecalho(grid_frame, 2, 7, "Comando", tk.Entry, {"textvariable": self.campos_dict["Comando"], "font": FONTE_PEQUENA, "relief": "flat"}, label_width=8, columnspan=2)

        opcoes_tele = ["", "sim", "não"]
        tele_combo = self._criar_celula_cabecalho(grid_frame, 2, 9, "Telecontrolado", ttk.Combobox, {"textvariable": self.campos_dict["Telecontrolado"], "values": opcoes_tele, "state": "readonly", "font": FONTE_PEQUENA, "style": 'Arrowless.TCombobox'}, label_width=12, columnspan=2)
        tele_combo.bind("<Button-1>", self.open_dropdown_on_click)

        self._criar_celula_cabecalho(grid_frame, 2, 11, "Siom", tk.Entry, {"textvariable": self.campos_dict["Siom"], "font": FONTE_PEQUENA, "relief": "flat"}, label_width=4, columnspan=1)
    ...
    def _criar_novas_tabelas_parametros(self, parent):
        ...
        tk.Label(coluna_esquerda, text="Parâmetros do Coordenograma do Cliente", font=FONTE_PADRAO, relief="solid", bd=1).grid(row=0, column=0, columnspan=3, sticky='nsew')
        tk.Label(coluna_esquerda, text="", font=FONTE_PADRAO, relief="solid", bd=1).grid(row=1, column=0, sticky='nsew')
        tk.Label(coluna_esquerda, text="Consumo", font=FONTE_PADRAO, relief="solid", bd=1).grid(row=1, column=1, sticky='nsew')
        tk.Label(coluna_esquerda, text="Injeção", font=FONTE_PADRAO, relief="solid", bd=1).grid(row=1, column=2, sticky='nsew')
        ...
        for i, label in enumerate(labels_coord[:4]):
            tk.Label(coluna_esquerda, text=label, font=FONTE_PADRAO, relief="solid", bd=1, anchor='w', width=20).grid(row=i+2, column=0, sticky='nsew')
            tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_consumo[i]], font=FONTE_PADRAO, justify='center', relief='solid', bd=1, width=12).grid(row=i+2, column=1, sticky='nsew')
            tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_injecao[i]], font=FONTE_PADRAO, justify='center', relief='solid', bd=1, width=12).grid(row=i+2, column=2, sticky='nsew')
        
        tk.Label(coluna_esquerda, text=labels_coord[4], font=FONTE_PADRAO, relief="solid", bd=1, anchor='w').grid(row=6, column=0, sticky='nsew')
        tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_consumo[4]], font=FONTE_PADRAO, justify='center', relief='solid', bd=1).grid(row=6, column=1, sticky='nsew')
        tk.Label(coluna_esquerda, text="", font=FONTE_PADRAO, relief="solid", bd=1).grid(row=6, column=2, sticky='nsew')

        tk.Label(coluna_esquerda, text=labels_coord[5], font=FONTE_PADRAO, relief="solid", bd=1, anchor='w').grid(row=7, column=0, sticky='nsew')
        tk.Label(coluna_esquerda, text="", font=FONTE_PADRAO, relief="solid", bd=1).grid(row=7, column=1, sticky='nsew')
        tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_injecao[4]], font=FONTE_PADRAO, justify='center', relief='solid', bd=1).grid(row=7, column=2, sticky='nsew')

        coluna_esquerda.grid_rowconfigure(8, minsize=10)

        # Tabela 2: Equipamento a Montante
        tk.Label(coluna_esquerda, text="Equipamento a Montante", font=FONTE_PADRAO, relief="solid", bd=1).grid(row=9, column=0, columnspan=2, sticky='nsew')
        ...
        for i, (label, var) in enumerate(params_montante):
            tk.Label(coluna_esquerda, text=label, font=FONTE_PADRAO, relief="solid", bd=1, anchor='w', width=20).grid(row=i+10, column=0, sticky='nsew')
            tk.Entry(coluna_esquerda, textvariable=var, font=FONTE_PADRAO, justify='center', relief='solid', bd=1, width=12).grid(row=i+10, column=1, sticky='nsew')

        container_tensao_corrente = tk.Frame(coluna_direita, bg='white')
        container_tensao_corrente.pack(anchor='w', pady=(0,10))

        # Tabela 3: Tensão
        f3 = tk.Frame(container_tensao_corrente, bg='white')
        ...
        for i, (label, var) in enumerate(params_tensao):
            tk.Label(f3, text=label, font=FONTE_PADRAO, bg='white', relief="solid", bd=1, width=24, anchor='w').grid(row=i, column=0, sticky="nsew")
            tk.Entry(f3, textvariable=var, font=FONTE_PADRAO, justify="center", relief="solid", bd=1, width=16).grid(row=i, column=1, sticky="nsew")
        
        # Tabela 5: Corrente de Falta
        f5 = tk.Frame(container_tensao_corrente, bg='white')
        f5.pack(side='left', anchor='n', padx=(20, 0))
        ...
        for i in range(15): coluna_esquerda.grid_rowconfigure(i, minsize=30)
        for i in range(4): f3.grid_rowconfigure(i, minsize=30)
        for i in range(3): f4.grid_rowconfigure(i, minsize=30)
        for i in range(4): f5.grid_rowconfigure(i, minsize=30)
