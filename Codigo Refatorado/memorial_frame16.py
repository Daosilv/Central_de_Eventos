...
class MemorialCalculoFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.campos_dict = { 
            ...
            "cabo_critico_nominal": tk.StringVar(), "cabo_critico_admissivel": tk.StringVar(),
            ...
            "eqpto_montante_numero": tk.StringVar(), "eqpto_montante_pickup_fase_g1": tk.StringVar(), "eqpto_montante_pickup_fase_g2": tk.StringVar(),
            "eqpto_montante_pickup_terra_g1": tk.StringVar(), "eqpto_montante_pickup_terra_g2": tk.StringVar(),
        }
        ...
        self.after(100, self._set_fixed_spacer_width)
    ...
    def get_db_columns():
        colunas_principais = [
            ...
            "cabo_critico_nominal", "cabo_critico_admissivel",

            "corrente_falta_fase_cc_max", "corrente_falta_fase_sens_princ", "corrente_falta_terra_sens_princ",

            "eqpto_montante_numero", "eqpto_montante_pickup_fase_g1", "eqpto_montante_pickup_fase_g2",
            "eqpto_montante_pickup_terra_g1", "eqpto_montante_pickup_terra_g2",
        ]
        colunas_grupos = []
        for i in range(1, 4):
    ...
    def criar_layout_principal(self):
        ...
        self.content_holder_frame.grid_columnconfigure(0, weight=0)
        self.content_holder_frame.grid_columnconfigure(1, weight=0)
        self.content_holder_frame.grid_columnconfigure(2, weight=0)
        
        self.content_holder_frame.grid_rowconfigure(0, weight=0)
        self.content_holder_frame.grid_rowconfigure(1, weight=1)

        self.header_frame = tk.Frame(self.content_holder_frame, bg='white')
        self.header_frame.grid(row=0, column=0, columnspan=2, sticky="nsew", padx=(0, 10))

        self.main_form_frame = tk.Frame(self.content_holder_frame, bd=1, bg='white')
        self.main_form_frame.grid(row=1, column=0, sticky="nsew", padx=(0, 10))

        sidebar_frame = tk.LabelFrame(self.content_holder_frame, text="Histórico", width=SIDEBAR_WIDTH, padx=5, pady=5, bg='white')
        sidebar_frame.grid(row=0, column=2, rowspan=2, sticky="ns", pady=10)
    ...
    def _set_fixed_spacer_width(self):
        self.update_idletasks()
        screen_width = self.winfo_screenwidth()
        col1_width = self.header_frame.winfo_width()
        col3_width = SIDEBAR_WIDTH
        
        spacer_width = screen_width - col1_width - col3_width - 70
        spacer_width = max(1, spacer_width)

        self.content_holder_frame.grid_columnconfigure(1, minsize=spacer_width)
    ...
    def _criar_layout_cabecalho(self):
        ...
        tk.Entry(row1_frame, textvariable=self.campos_dict["Data"], relief="solid", bd=1, font=FONTE_PADRAO, width=12).pack(side="left", padx=(0,5))
        tk.Label(row1_frame, text="NS", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        tk.Entry(row1_frame, textvariable=self.campos_dict["NS"], relief="solid", bd=1, font=FONTE_PADRAO, width=15).pack(side="left", padx=(0,5))
        tk.Label(row1_frame, text="Eqpto", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        tk.Entry(row1_frame, textvariable=self.campos_dict["Eqpto"], relief="solid", bd=1, font=FONTE_PADRAO, width=15).pack(side="left", padx=(0,5))
        ...
        tk.Label(row1_frame, text="Condição", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        self.create_bordered_combobox(row1_frame, self.campos_dict["Condição"], opcoes_condicao, 20).pack(side="left", padx=(0,5))
        tk.Label(row1_frame, text="Coord", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        tk.Entry(row1_frame, textvariable=self.campos_dict["Coord"], relief="solid", bd=1, font=FONTE_PADRAO, width=20).pack(side="left", padx=(0,5))
        ...
        self.create_bordered_combobox(row2_frame, self.campos_dict["Malha"], opcoes_malha, 20).pack(side="left", padx=(0,5))
        tk.Label(row2_frame, text="Alimentador", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        tk.Entry(row2_frame, textvariable=self.campos_dict["Alimentador"], relief="solid", bd=1, font=FONTE_PADRAO, width=15).pack(side="left", padx=(0,5))
        opcoes_motivador = ["", "Instalação", "Reajuste", "Relocação"]
        tk.Label(row2_frame, text="Motivador", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        self.create_bordered_combobox(row2_frame, self.campos_dict["Motivador"], opcoes_motivador, 20).pack(side="left", padx=(0,5))
        tk.Label(row2_frame, text="Bypass", relief="solid", bd=1, font=FONTE_PADRAO, bg='white').pack(side="left")
        opcoes_bypass = ["", "6", "8", "10", "12", "15", "20", "25", "30"]
        bypass1_cell = tk.Frame(row2_frame, relief="solid", bd=1, bg='white')
        ...
        tk.Label(row2_frame, text="-", bd=0, font=FONTE_PADRAO, bg='white').pack(side="left", padx=2)
        tk.Entry(row2_frame, textvariable=self.campos_dict["Bypass_2"], width=10, relief="solid", bd=1, font=FONTE_PADRAO).pack(side="left", padx=(0,5))
        tk.Label(row2_frame, text="Endereço", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        tk.Entry(row2_frame, textvariable=self.campos_dict["Endereço"], relief="solid", bd=1, font=FONTE_PADRAO).pack(side="left", padx=(0,5), fill="x", expand=True)
        ...
        self.create_bordered_combobox(row3_frame, self.campos_dict["Mídia"], opcoes_midia, 15).pack(side="left", padx=(0,5))
        ...
        tk.Label(row3_frame, text="Fabricante", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        self.create_bordered_combobox(row3_frame, self.campos_dict["Fabricante"], opcoes_fabricante, 25).pack(side="left", padx=(0,5))
        tk.Label(row3_frame, text="Modelo", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        tk.Entry(row3_frame, textvariable=self.campos_dict["Modelo"], relief="solid", bd=1, font=FONTE_PADRAO, width=20).pack(side="left", padx=(0,5))
        tk.Label(row3_frame, text="Comando", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        tk.Entry(row3_frame, textvariable=self.campos_dict["Comando"], relief="solid", bd=1, font=FONTE_PADRAO, width=20).pack(side="left", padx=(0,5))
        opcoes_telecontrolado = ["", "sim", "não"]
        tk.Label(row3_frame, text="Telecontrolado", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        self.create_bordered_combobox(row3_frame, self.campos_dict["Telecontrolado"], opcoes_telecontrolado, 8).pack(side="left", padx=(0,5))
        tk.Label(row3_frame, text="Siom", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        tk.Entry(row3_frame, textvariable=self.campos_dict["Siom"], relief="solid", bd=1, font=FONTE_PADRAO, width=15).pack(side="left", padx=(0,5))

    def criar_layout_formulario(self):
        btn_voltar = ttk.Button(self.main_form_frame, text="< Fechar Módulo", command=self.parent_window.destroy)
    ...
    def _criar_novas_tabelas_parametros(self, parent):
        ...
        # Container para a coluna da esquerda que usará um grid único para alinhar as tabelas
        coluna_esquerda = tk.Frame(container_principal, bg='white')
        coluna_esquerda.grid(row=0, column=0, sticky='nw')
        
        # Container para a coluna da direita
        coluna_direita = tk.Frame(container_principal, bg='white')
        coluna_direita.grid(row=0, column=1, sticky='nw', padx=(20, 0))

        # --- Construção das Tabelas na Coluna Esquerda (em um grid único) ---
        
        # Tabela 1: Parâmetros do Coordenograma do Cliente
        tk.Label(coluna_esquerda, text="Parâmetros do Coordenograma do Cliente", font=FONTE_PEQUENA, relief="solid", bd=1).grid(row=0, column=0, columnspan=3, sticky='nsew')
        tk.Label(coluna_esquerda, text="", font=FONTE_PEQUENA, relief="solid", bd=1).grid(row=1, column=0, sticky='nsew')
        tk.Label(coluna_esquerda, text="Consumo", font=FONTE_PEQUENA, relief="solid", bd=1).grid(row=1, column=1, sticky='nsew')
        tk.Label(coluna_esquerda, text="Injeção", font=FONTE_PEQUENA, relief="solid", bd=1).grid(row=1, column=2, sticky='nsew')
        ...
        for i, label in enumerate(labels_coord[:4]):
            tk.Label(coluna_esquerda, text=label, font=FONTE_PEQUENA, relief="solid", bd=1, anchor='w', width=25).grid(row=i+2, column=0, sticky='nsew')
            tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_consumo[i]], font=FONTE_PEQUENA, justify='center', relief='solid', bd=1, width=15).grid(row=i+2, column=1, sticky='nsew')
            tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_injecao[i]], font=FONTE_PEQUENA, justify='center', relief='solid', bd=1, width=15).grid(row=i+2, column=2, sticky='nsew')
        
        tk.Label(coluna_esquerda, text=labels_coord[4], font=FONTE_PEQUENA, relief="solid", bd=1, anchor='w').grid(row=6, column=0, sticky='nsew')
        tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_consumo[4]], font=FONTE_PEQUENA, justify='center', relief='solid', bd=1).grid(row=6, column=1, sticky='nsew')
        tk.Label(coluna_esquerda, text="", font=FONTE_PEQUENA, relief="solid", bd=1).grid(row=6, column=2, sticky='nsew')

        tk.Label(coluna_esquerda, text=labels_coord[5], font=FONTE_PEQUENA, relief="solid", bd=1, anchor='w').grid(row=7, column=0, sticky='nsew')
        tk.Label(coluna_esquerda, text="", font=FONTE_PEQUENA, relief="solid", bd=1).grid(row=7, column=1, sticky='nsew')
        tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_injecao[4]], font=FONTE_PEQUENA, justify='center', relief='solid', bd=1).grid(row=7, column=2, sticky='nsew')

        # Espaçador entre as tabelas
        coluna_esquerda.grid_rowconfigure(8, minsize=10)

        # Tabela 2: Equipamento a Montante
        tk.Label(coluna_esquerda, text="Equipamento a Montante", font=FONTE_PEQUENA, relief="solid", bd=1).grid(row=9, column=0, columnspan=2, sticky='nsew')
        
        params_montante = [
            ("Número do Equipamento", self.campos_dict["eqpto_montante_numero"]),
            ("Pick-up Fase - G1 (A)", self.campos_dict["eqpto_montante_pickup_fase_g1"]),
            ("Pick-up Fase - G2 (A)", self.campos_dict["eqpto_montante_pickup_fase_g2"]),
            ("Pick-up Terra - G1 (A)", self.campos_dict["eqpto_montante_pickup_terra_g1"]),
            ("Pick-up Terra - G2 (A)", self.campos_dict["eqpto_montante_pickup_terra_g2"]),
        ]
        for i, (label, var) in enumerate(params_montante):
            tk.Label(coluna_esquerda, text=label, font=FONTE_PEQUENA, relief="solid", bd=1, anchor='w', width=25).grid(row=i+10, column=0, sticky='nsew')
            tk.Entry(coluna_esquerda, textvariable=var, font=FONTE_PEQUENA, justify='center', relief='solid', bd=1, width=15).grid(row=i+10, column=1, sticky='nsew')

        # --- Construção das Tabelas na Coluna Direita ---
        # Tabela 3: Tensão
        f3 = tk.Frame(coluna_direita, bg='white')
        ...
        for i, (label, var) in enumerate(params_tensao):
            tk.Label(f3, text=label, font=FONTE_PEQUENA, bg='white', relief="solid", bd=1, width=30, anchor='w').grid(row=i, column=0, sticky="nsew")
            tk.Entry(f3, textvariable=var, font=FONTE_PEQUENA, justify="center", relief="solid", bd=1, width=20).grid(row=i, column=1, sticky="nsew")
        
        # Tabela 4: Cabo Crítico
        f4 = tk.Frame(coluna_direita, bg='white')
        f4.pack(anchor='w', pady=(0, 10))
        tk.Label(f4, text="Cabo crítico no trecho", font=FONTE_PEQUENA, bg='white', relief="solid", bd=1).grid(row=0, column=0, columnspan=2, sticky='nsew')
        tk.Label(f4, text="Nominal [A]", font=FONTE_PEQUENA, bg='white', relief="solid", bd=1, width=15).grid(row=1, column=0, sticky='nsew')
        tk.Label(f4, text="Admissível [A]", font=FONTE_PEQUENA, bg='white', relief="solid", bd=1, width=15).grid(row=1, column=1, sticky='nsew')
        tk.Entry(f4, textvariable=self.campos_dict["cabo_critico_nominal"], font=FONTE_PEQUENA, justify="center", relief="solid", bd=1).grid(row=2, column=0, sticky="nsew")
        tk.Entry(f4, textvariable=self.campos_dict["cabo_critico_admissivel"], font=FONTE_PEQUENA, justify="center", relief="solid", bd=1).grid(row=2, column=1, sticky="nsew")

        # Tabela 5: Corrente de Falta
        f5 = tk.Frame(coluna_direita, bg='white')
        f5.pack(anchor='w')
        tk.Label(f5, text="Corrente de Falta", font=FONTE_PEQUENA, bg='white', relief="solid", bd=1).grid(row=0, column=0, columnspan=2, sticky='nsew')
        params_corrente = [
        ...
        for i, (label, var) in enumerate(params_corrente):
            tk.Label(f5, text=label, font=FONTE_PEQUENA, bg='white', relief="solid", bd=1, width=35, anchor='w').grid(row=i+1, column=0, sticky="nsew")
            tk.Entry(f5, textvariable=var, font=FONTE_PEQUENA, justify="center", relief="solid", bd=1, width=15).grid(row=i+1, column=1, sticky="nsew")
