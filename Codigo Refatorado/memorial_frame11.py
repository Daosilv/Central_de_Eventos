...
SIDEBAR_WIDTH = 450 # Largura aumentada para melhor visualização
...
class MemorialCalculoFrame(tk.Frame):
    ...
    def _criar_tabela_tensao_corrente(self, parent):
        ...
        for i, (label_text, var) in enumerate(campos):
            tk.Label(frame_tabela, text=label_text, font=FONTE_PEQUENA, bg='white', relief="solid", bd=1, width=30).grid(row=i, column=0, sticky="nsew")
    ...
    def _criar_tabela_cc_sensibilidade(self, parent):
        ...
        for i, (label_text, var) in enumerate(campos):
            tk.Label(frame_tabela, text=label_text, font=FONTE_PEQUENA, bg='white', relief="solid", bd=1, width=40).grid(row=i, column=0, sticky="nsew")
            tk.Entry(frame_tabela, textvariable=var, font=FONTE_PEQUENA, justify="center", relief="solid", bd=1, width=10).grid(row=i, column=1, sticky="nsew")
    ...
    def _criar_tabela_cabos_protecao(self, parent):
        ...
        tk.Label(frame_cabos, text="Bitola do cabo crítico no trecho", font=FONTE_PEQUENA, bg='white', relief="solid", bd=1).grid(row=0, column=0, sticky='nsew')
        tk.Label(frame_cabos, text="Cabo", font=FONTE_PEQUENA, bg='white', relief="solid", bd=1).grid(row=1, column=0, sticky='nsew')
        ...
        tk.Entry(frame_cabos, state='disabled', font=FONTE_PEQUENA, justify="center", relief="solid", bd=1, width=15).grid(row=2, column=1, sticky="nsew")
        tk.Entry(frame_cabos, state='disabled', font=FONTE_PEQUENA, justify="center", relief="solid", bd=1, width=15).grid(row=2, column=2, sticky="nsew")
        ...
        for i, (label_text, var) in enumerate(campos_protecao):
            tk.Label(frame_protecao, text=label_text, font=FONTE_PEQUENA, bg='white', relief="solid", bd=1, width=50).grid(row=i, column=0, sticky="nsew")
    ...
    def inicializar_banco(self):
        try:
            ...
            with sqlite3.connect(caminho_bd_ativo) as conexao:
                ...
                novas_colunas = ['observacao', 'manobra_efetiva', 'anexos', 'tensao_kv', 'transformadores_fase_a', 'transformadores_fase_b', 'transformadores_fase_c', 'fase_cc_max', 'fase_sens_princ', 'terra_sens_princ', 'bitola_cabo_critico', 'cabo_nominal_a', 'cabo_admissivel_a', 'pickup_protecao_terra', 'tempo_protecao_terra']
    ...
    def reconstruir_lista_ajustes(self, *args):
        ...
        try:
            ...
            with sqlite3.connect(caminho_bd_ativo) as conexao:
                cursor = conexao.cursor()
                cursor.execute("SELECT eqpto, ns, data, alimentador FROM cadastros WHERE eqpto != '' ORDER BY data_registro DESC LIMIT 50")
        ...
        for item_data in ajustes_filtrados:
            eqpto, ns, data, alimentador = item_data
    ...
    def reconstruir_lista_rascunhos(self, *args):
        ...
        try:
            ...
            with sqlite3.connect(caminho_bd_ativo) as conexao:
                cursor = conexao.cursor()
                cursor.execute("SELECT eqpto, ns, data, alimentador FROM rascunhos WHERE eqpto != '' ORDER BY data_registro DESC")
