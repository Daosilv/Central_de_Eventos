...
class MemorialCalculoFrame(tk.Frame):
    ...
    def criar_layout_principal(self):
        ...
        # --- ALTERAÇÃO APLICADA AQUI: Estrutura de "Ajustes Recentes" refatorada ---
        ...
        # --- ALTERAÇÃO APLICADA AQUI: Borda adicionada ao container dos rascunhos ---
    ...
    def reconstruir_lista_ajustes(self, *args):
        ...
        ajustes_filtrados = [a for a in self.lista_ajustes_completa if termo_busca in a.lower()]

        for eqpto in ajustes_filtrados:
            row = tk.Frame(self.ajustes_items_frame, bg='white'); row.pack(fill='x', expand=True, pady=1)
            lbl = tk.Label(row, text=eqpto, anchor='w', bg='white', padx=4)
            lbl.pack(side='left', fill='x', expand=True)
            lbl.bind("<Double-1>", lambda e, eq=eqpto: self.carregar_ajuste_pelo_nome(eq))
    ...
    def reconstruir_lista_rascunhos(self, *args):
        ...
        for eqpto in rascunhos_filtrados:
            ...
            lbl = tk.Label(row, text=eqpto, anchor='w', bg='white')
            lbl.grid(row=0, column=1, sticky='nsew', padx=(5, 0))
            lbl.bind("<Double-1>", lambda e, eq=eqpto: self.carregar_rascunho_pelo_nome(eq))
            tk.Button(row, text="X", fg="red", relief="flat", cursor="hand2", font=("Helvetica", 8, "bold"), command=lambda e=eqpto: self.deletar_rascunho_individual(e), bg='white').grid(row=0, column=2, sticky='e', padx=(0,5))
