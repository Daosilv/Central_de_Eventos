...
class MapEditDialog(tk.Toplevel):
    def __init__(self, parent, title, initial_data):
        ...
        tk.Entry(frame, textvariable=self.campo_var, width=40, state='readonly').grid(row=0, column=1, sticky='ew')
    ...
    def on_save(self):
        self.result = {
            "campo_app": self.campo_var.get(),
            "aba_excel": self.aba_var.get().strip(),
            "celula_excel": self.celula_var.get().strip()
        }
        self.destroy()
...
class TabMapeamento(tk.Frame):
    ...
    def criar_widgets(self):
        ...
        map_list_frame = tk.LabelFrame(main_pane, text="Mapas", width=250)
        main_pane.add(map_list_frame, weight=1)
        map_list_frame.grid_rowconfigure(0, weight=1)
        map_list_frame.grid_columnconfigure(0, weight=1)

        self.map_listbox = tk.Listbox(map_list_frame, exportselection=False)
        self.map_listbox.grid(row=0, column=0, columnspan=2, sticky='nsew')
        self.map_listbox.bind('<<ListboxSelect>>', self.on_map_select)

        map_buttons_frame = tk.Frame(map_list_frame)
        map_buttons_frame.grid(row=1, column=0, columnspan=2, sticky='ew')
        ttk.Button(map_buttons_frame, text="Adicionar", command=self.adicionar_mapa).pack(side='left', expand=True, fill='x')
        ttk.Button(map_buttons_frame, text="Renomear", command=self.renomear_mapa).pack(side='left', expand=True, fill='x')
        ttk.Button(map_buttons_frame, text="Remover", command=self.remover_mapa).pack(side='left', expand=True, fill='x')
        ttk.Button(map_buttons_frame, text="Exportar", command=self.exportar_mapeamento_para_excel).pack(side='left', expand=True, fill='x')

        tree_frame = tk.LabelFrame(main_pane, text="Mapeamento de Campos para o Excel")
        ...
        self.tree.bind("<Double-1>", self.on_edit_mapping)
        
        botoes_frame = tk.Frame(tree_frame)
        botoes_frame.grid(row=1, column=0, sticky="ew")
        ttk.Button(botoes_frame, text="Adicionar Múltiplos", command=self.adicionar_mapeamentos).pack(side='left', padx=5, pady=5)
        ttk.Button(botoes_frame, text="Remover Selecionado", command=self.remover_mapeamento).pack(side='left', padx=5, pady=5)
        ttk.Button(botoes_frame, text="Remover Tudo", command=self.remover_todos_mapeamentos).pack(side='left', padx=5, pady=5)

    def carregar_mapas(self):
        self.map_listbox.delete(0, tk.END)
        self.map_id_map.clear()
        mapas = self.controller.get_all_map_names()
        if not mapas: return
        for mapa in mapas:
            idx = self.map_listbox.size()
            self.map_listbox.insert(tk.END, mapa['nome'])
            self.map_id_map[idx] = mapa['id']
        
        if self.map_listbox.size() > 0:
    ...
    def renomear_mapa(self):
        ...
        if not sel: return
        map_id = self.map_id_map.get(sel[0])
        nome_atual = self.map_listbox.get(sel[0])
        novo_nome = simpledialog.askstring("Renomear Mapa", "Novo nome:", initialvalue=nome_atual, parent=self)
        if novo_nome and novo_nome.strip():
            self.controller.rename_map_name(map_id, novo_nome.strip())
            self.carregar_mapas()
    ...
    def remover_mapa(self):
        ...
        if not sel: return
        map_id = self.map_id_map.get(sel[0])
        nome = self.map_listbox.get(sel[0])
        if messagebox.askyesno("Confirmar", f"Deseja remover o mapa '{nome}' e todos os seus mapeamentos?", icon='warning', parent=self):
            self.controller.delete_map(map_id)
            self.carregar_mapas()

    def carregar_mapeamento(self, map_id):
        self.tree.delete(*self.tree.get_children())
        mapeamentos = self.controller.get_mappings_for_map(map_id)
    ...
    def adicionar_mapeamentos(self):
        sel = self.map_listbox.curselection()
        if not sel:
            messagebox.showwarning("Atenção", "Selecione um mapa à esquerda antes de adicionar mapeamentos.", parent=self)
            return
        map_id = self.map_id_map.get(sel[0])
        
        dialog = MultiMapDialog(self, title="Adicionar Mapeamentos")
        if dialog.result:
            self.controller.add_mappings_to_map(map_id, dialog.result)
            self.carregar_mapeamento(map_id)
    ...
    def remover_todos_mapeamentos(self):
        sel = self.map_listbox.curselection()
        if not sel:
            messagebox.showwarning("Atenção", "Selecione um mapa para limpar.", parent=self)
            return
        map_id = self.map_id_map.get(sel[0])
        if messagebox.askyesno("Confirmar Remoção Total", "Tem certeza que deseja remover TODOS os mapeamentos deste mapa?", icon='warning', parent=self):
            self.controller.deletar_mapeamentos_por_mapa(map_id)
            self.on_map_select(None)
    ...
    def on_edit_mapping(self, event):
        ...
        initial_data = {
            'campo': item_values[0],
            'aba': item_values[1],
            'celula': item_values[2]
        }
        
        dialog = MapEditDialog(self, "Editar Mapeamento", initial_data=initial_data)
        if dialog.result:
            self.controller.update_mapping(int(selected_id), dialog.result)
            self.on_map_select(None)

    def exportar_mapeamento_para_excel(self):
        sel = self.map_listbox.curselection()
        if not sel:
            messagebox.showwarning("Atenção", "Selecione um mapa para exportar.", parent=self)
            return
        ...
        mapeamentos = self.controller.get_mappings_for_map(map_id)
        if not mapeamentos:
            messagebox.showinfo("Vazio", "Este mapa não possui mapeamentos para exportar.", parent=self)
            return
            
        filepath = filedialog.asksaveasfilename(
            defaultextension=".xlsx",
            filetypes=[("Excel Workbook", "*.xlsx")],
            title="Salvar Mapeamento Como...",
            initialfile=f"Mapeamento_{map_name}.xlsx"
        )
        ...
        try:
            ...
            ws.append(['Campo no Aplicativo', 'Aba no Excel', 'Célula no Excel'])
            
            for item in mapeamentos:
                ws.append([item['campo_app'], item['aba_excel'], item['celula_excel']])

            wb.save(filepath)
            messagebox.showinfo("Sucesso", f"Mapeamento exportado com sucesso para:\n{filepath}", parent=self)
