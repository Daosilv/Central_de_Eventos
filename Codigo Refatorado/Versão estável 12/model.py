...
class DatabaseModel:
    ...
    def create_tables(self, columns_for_creation):
        ...
        # Estrutura antiga da tabela de mapeamento (será migrada)
        self._execute("CREATE TABLE IF NOT EXISTS mapeamento_excel (id INTEGER PRIMARY KEY AUTOINCREMENT, campo_app TEXT NOT NULL UNIQUE, aba_excel TEXT NOT NULL, celula_excel TEXT NOT NULL)", commit=True)
        ...
        # --- NOVA TABELA PARA NOMES DOS MAPAS ---
        ...
        self._migrate_mappings_table() # <-- CHAMADA PARA A MIGRAÇÃO
    ...
    # --- LÓGICA DE MIGRAÇÃO DO MAPEAMENTO ANTIGO ---
    ...
    def add_mappings_to_map(self, map_id, mappings_list):
        ...
        query = "INSERT OR REPLACE INTO mapeamento_excel (mapa_id, campo_app, aba_excel, celula_excel) VALUES (?, ?, ?, ?)"
        # Nota: INSERT OR REPLACE depende de um UNIQUE index. A lógica precisa ser ajustada
        # Removendo a constraint UNIQUE de campo_app e usando um índice composto (mapa_id, campo_app)
        # Por simplicidade, vamos deletar e inserir.
    ...
    def update_mapping(self, mapping_id, data):
        query = "UPDATE mapeamento_excel SET aba_excel = ?, celula_excel = ? WHERE id = ?"
        params = (data['aba_excel'], data['celula_excel'], mapping_id)
    ...
    # --- Demais métodos (usuários, observações, etc.) ---
    ...
    def get_recent_equipments_with_status(self, limit=50):
        query = "SELECT c.eqpto, c.ns, c.data_registro, c.alimentador, COALESCE(a.status, latest_h.status, 'Pendente') as status FROM (SELECT *, ROW_NUMBER() OVER(PARTITION BY eqpto ORDER BY data_registro DESC) as rn FROM cadastros WHERE eqpto != '') c LEFT JOIN ajustes a ON c.eqpto = a.eqpto LEFT JOIN (SELECT eqpto, status, ROW_NUMBER() OVER(PARTITION BY eqpto ORDER BY data_conclusao DESC) as rn FROM historico_ajustes) latest_h ON c.eqpto = latest_h.eqpto AND latest_h.rn = 1 WHERE c.rn = 1 ORDER BY c.data_registro DESC LIMIT ?"
    ...
    def get_pending_adjustments(self):
        return self._execute("SELECT a.eqpto, a.status, c.autor, c.data, c.malha, c.alimentador, c.fabricante, c.comando FROM ajustes a JOIN (SELECT *, ROW_NUMBER() OVER(PARTITION BY eqpto ORDER BY data_registro DESC) as rn FROM cadastros) c ON a.eqpto = c.eqpto WHERE c.rn = 1 ORDER BY a.data_adicao DESC", fetch='all')
