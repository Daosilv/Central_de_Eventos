...
class MemorialCalculoFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        style.configure("TCheckbutton", font=FONTE_PADRAO, background='white')
        ...
        self.campos_dict = { "Autor": tk.StringVar(), "Eqpto": tk.StringVar(), "Malha": tk.StringVar(), "Condição": tk.StringVar(), "Motivador": tk.StringVar(), "Alimentador": tk.StringVar(), "NS": tk.StringVar(), "Siom": tk.StringVar(), "Data": tk.StringVar(value=datetime.now().strftime('%d/%m/%Y')), "Coord": tk.StringVar(), "Endereço": tk.StringVar(), "Mídia": tk.StringVar(), "Fabricante": tk.StringVar(), "Modelo": tk.StringVar(), "Comando": tk.StringVar(), "Telecontrolado": tk.StringVar(value="sim"), "Bypass_1": tk.StringVar(), "Bypass_2": tk.StringVar(),
                            "tensao_kv": tk.StringVar(), "corrente_fase_a": tk.StringVar(), "corrente_fase_b": tk.StringVar(), "corrente_fase_c": tk.StringVar(),
                            "fase_cc_max": tk.StringVar(), "fase_sens_princ": tk.StringVar(), "terra_sens_princ": tk.StringVar(),
                            "vinculado_acessante": tk.StringVar(value="não")}
    ...
    def get_db_columns():
        ...
        colunas_dict = { "Autor": "", "Eqpto": "", "Malha": "", "Condição": "", "Motivador": "", 
                         "Alimentador": "", "NS": "", "Siom": "", "Data": "", "Coord": "", "Endereço": "", 
                         "Mídia": "", "Fabricante": "", "Modelo": "", "Comando": "", "Telecontrolado": "", 
                         "Bypass_1": "", "Bypass_2": "", "tensao_kv": "", "corrente_fase_a": "", 
                         "corrente_fase_b": "", "corrente_fase_c": "", "fase_cc_max": "", "fase_sens_princ": "", 
                         "terra_sens_princ": "", "vinculado_acessante": "" }
        ...
        colunas_db.append('manobra_efetiva')
        return colunas_db
    ...
    def _anexar_arquivo(self, group_index):
        # Lógica para anexar arquivo pode ser adicionada aqui
        messagebox.showinfo("Anexar Arquivo", f"Função para anexar arquivo para o Grupo {group_index}", parent=self.parent_window)
    ...
    def criar_layout_formulario(self):
        ...
        tk.Label(row_bypass_frame, text="Bypass", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        ...
        row_vinculado_frame = tk.Frame(frame_campos, bg='white'); row_vinculado_frame.pack(fill="x", pady=1)
        tk.Label(row_vinculado_frame, text="Vinculado a Acessante", relief="solid", bd=1, anchor="e", font=FONTE_PADRAO, bg='white').pack(side="left")
        opcoes_vinculado = ["sim", "não"]
        self.create_bordered_combobox(row_vinculado_frame, self.campos_dict["vinculado_acessante"], opcoes_vinculado, 10).pack(side="left", padx=(0,5))
        ...
        manobra_frame = tk.Frame(self.main_form_frame, bg='white'); manobra_frame.pack(pady=(10, 5), padx=10, anchor='w')
    ...
    def _criar_tabela_tensao_corrente(self, parent):
        ...
        frame_tabela.pack(side='left', anchor='n', padx=(0, 20))
        
        campos = [
            ("Tensão [kV]", self.campos_dict["tensao_kv"]),
            ("Corrente Fase A [A]", self.campos_dict["corrente_fase_a"]),
            ("Corrente Fase B [A]", self.campos_dict["corrente_fase_b"]),
            ("Corrente Fase C [A]", self.campos_dict["corrente_fase_c"])
        ]
        
        for i, (label_text, var) in enumerate(campos):
            label_frame = tk.Frame(frame_tabela, bd=1, relief="solid", bg='white')
            label_frame.grid(row=i, column=0, sticky="nsew")
            tk.Label(label_frame, text=label_text, font=FONTE_PADRAO, bg='white', anchor='center').pack(fill="both", expand=True)
            
            entry_frame = tk.Frame(frame_tabela, bd=1, relief="solid", bg='white')
            entry_frame.grid(row=i, column=1, sticky="nsew")
            tk.Entry(entry_frame, textvariable=var, font=FONTE_PADRAO, justify="center", bd=0).pack(fill="both", expand=True)

    def _criar_tabela_cc_sensibilidade(self, parent):
        frame_tabela = tk.Frame(parent, bg='white')
        frame_tabela.pack(side='left', anchor='n')
        ...
        for i, (label_text, var) in enumerate(campos):
            label_frame = tk.Frame(frame_tabela, bd=1, relief="solid", bg='white')
            label_frame.grid(row=i, column=0, sticky="nsew")
            tk.Label(label_frame, text=label_text, font=FONTE_PADRAO, bg='white', width=30, anchor='center').pack(fill="both", expand=True)
            
            entry_frame = tk.Frame(frame_tabela, bd=1, relief="solid", bg='white')
            entry_frame.grid(row=i, column=1, sticky="nsew")
            tk.Entry(entry_frame, textvariable=var, font=FONTE_PADRAO, justify="center", bd=0, width=10).pack(fill="both", expand=True)
    ...
    def _construir_bloco_tabelas(self, parent, vars_storage, tables_storage, refs_storage, tipo_bloco):
        ...
        for i in range(1, 4):
            # Cria um container para a linha da tabela e o botão
            row_container = tk.Frame(parent, bg='white')
            
            tabela_frame, ref_cell, group_vars = self.criar_tabela_grupo(row_container, f"  Grupo {i}  ")
            tabela_frame.pack(side='left', fill='both', expand=True)

            btn_anexo = tk.Button(row_container, text="+", font=("Helvetica", 10), relief="groove",
                                  width=3, height=1,
                                  command=lambda index=i: self._anexar_arquivo(index))
            btn_anexo.pack(side='left', padx=10, anchor='center')

            row_container.pack(pady=(0,5), fill='x', expand=True)
    ...
    def criar_tabela_grupo(self, parent, group_name):
        ...
        return frame_tabela, None, group_vars
