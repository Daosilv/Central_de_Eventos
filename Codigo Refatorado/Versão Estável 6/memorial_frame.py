...
class MemorialCalculoFrame(tk.Frame):
    ...
    def criar_layout_formulario(self):
        ...
        # --- Linha de botões com o novo botão "Gerar Doc" ---
        ...
        ttk.Button(botoes_frame, text="Gerar Doc", command=self.gerar_documento_excel).pack(side="left", padx=5)
    ...
    def _gerenciar_bloco_tabelas_replicado(self, *args):
        ...
        new_state = 'normal' if cond.lower() in ["acess inv", "acess s/inv"] else 'disabled'
    ...
    def salvar_rascunho(self):
        ...
        if not data.get("eqpto", "").strip():
            messagebox.showwarning("Campo Obrigatório", "O campo 'Eqpto' é obrigatório para salvar um rascunho.", parent=self.parent_window)
            return
    ...
    def reconstruir_lista_ajustes(self, *args):
        ...
        for i, item in enumerate(filtrados):
            ...
            cor_led = {'Andamento': '#f0ad4e', 'Aprovado': '#5cb85c', 'Reprovado': '#d9534f'}.get(status, 'grey')
            ...
            if item.get('data_registro'):
                try: data_formatada = datetime.strptime(item['data_registro'].split('.')[0], '%Y-%m-%d %H:%M:%S').strftime('%d/%m/%Y')
                except (ValueError, TypeError): data_formatada = "Data Inválida"
    ...
    def _replicar_sequencia_g1_para_g3(self, *args):
        try:
            self.vars_grupos['grupo_3']['sequencia'].set(self.vars_grupos['grupo_1']['sequencia'].get())
        except KeyError:
            pass
            
    # --- NOVO MÉTODO PARA CHAMAR O CONTROLADOR ---
    def gerar_documento_excel(self):
        self.controller.gerar_documento_excel(self)

    def deletar_rascunho_individual(self, eqpto):
        if messagebox.askyesno("Confirmar Exclusão", f"Deseja deletar o rascunho para '{eqpto}'?"):
            self.controller.deletar_rascunhos(self, [eqpto])

    def deletar_rascunhos_selecionados(self):
        a_deletar = [eqpto for eqpto, var in self.rascunho_check_vars.items() if var.get()]
        if not a_deletar:
            messagebox.showwarning("Nenhuma Seleção", "Nenhum rascunho selecionado para exclusão.", parent=self)
            return
        if messagebox.askyesno("Confirmar Exclusão", f"Deseja deletar os {len(a_deletar)} rascunhos selecionados?"):
            self.controller.deletar_rascunhos(self, a_deletar)

    def _toggle_selecionar_tudo_rascunhos(self):
        [var.set(self.selecionar_tudo_rascunhos_var.get()) for var in self.rascunho_check_vars.values()]
