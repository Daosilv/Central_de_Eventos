...
# --- INÍCIO DA ALTERAÇÃO: Nova janela para selecionar o mapa ---
class MapSelectDialog(simpledialog.Dialog):
    def __init__(self, parent, title, maps):
        self.maps = maps
...
class MemorialCalculoFrame(tk.Frame):
    ...
    def exportar_documento(self):
        # --- INÍCIO DA ALTERAÇÃO: Lógica de exportação atualizada ---
        ...
        dialog = MapSelectDialog(self.parent_window, "Selecionar Mapa de Exportação", maps)
        if dialog.result:
    ...
    def reconstruir_lista_rascunhos(self, *args):
        ...
        for i, item in enumerate(filtrados):
            ...
            if data_str:
                try:
                    # Tenta formatar a data que pode já estar em d/m/Y ou Y-m-d H:M:S
