...
class DetalhesWindow(tk.Toplevel):
    ...
    def criar_layout_visualizacao(self):
        ...
        for i in range(1, 4):
            self._criar_tabela_grupo_visualizacao(wrap_tabelas, f"  Grupo {i}  ", i)

        condicao = d.get("condição")
        if condicao in ["Acess Inv", "Acess s/Inv"]:
            frame_tabela_header_rep = tk.Frame(wrap_tabelas, bd=1, relief="solid")
            for c, minsize in enumerate(self.col_widths): frame_tabela_header_rep.grid_columnconfigure(c, minsize=minsize)
            for r in range(3): frame_tabela_header_rep.grid_rowconfigure(r, minsize=30, weight=1)
            self._cell(frame_tabela_header_rep, 0, 0, "Grupo Invertido = Grupo de Injeção", cs=9); self._cell(frame_tabela_header_rep, 1, 0, "  Grupos  ", rs=2)
            self._cell(frame_tabela_header_rep, 1, 1, "FASE", cs=4); self._cell(frame_tabela_header_rep, 1, 5, "TERRA", cs=4)
            self._cell(frame_tabela_header_rep, 2, 1, "  Pickup  "); self._cell(frame_tabela_header_rep, 2, 2, "  Sequência  "); self._cell(frame_tabela_header_rep, 2, 3, "  Curva Lenta  ")
            self._cell(frame_tabela_header_rep, 2, 4, "  Curva Rápida  "); self._cell(frame_tabela_header_rep, 2, 5, "  Pickup  "); self._cell(frame_tabela_header_rep, 2, 6, "  Sequência  ")
            self._cell(frame_tabela_header_rep, 2, 7, "  Curva Lenta  "); self._cell(frame_tabela_header_rep, 2, 8, "  Curva Rápida  ")
            frame_tabela_header_rep.pack(pady=(20, 5))
            
            for i in range(1, 4):
                self._criar_tabela_grupo_visualizacao(wrap_tabelas, f"  Grupo {i}  ", i, suffix="_rep")
    ...
    def _criar_tabela_grupo_visualizacao(self, parent, group_name, group_index, suffix=""):
        ...
        self._cell(frame_tabela, 0, 3, d.get(f'g{group_index}_curva_lenta_tipo{suffix}'))
        ...
        tk.Label(subframe_lenta, text=d.get(f'g{group_index}_curva_lenta_dial{suffix}'), font=FONTE_PADRAO, relief="solid", bd=1).grid(row=1, column=0, sticky="nsew")
        tk.Label(subframe_lenta, text=d.get(f'g{group_index}_curva_lenta_tadic{suffix}'), font=FONTE_PADRAO, relief="solid", bd=1).grid(row=1, column=1, sticky="nsew")
        ...
        tk.Label(subframe_rapida, text=d.get(f'g{group_index}_curva_rapida_dial{suffix}'), font=FONTE_PADRAO, relief="solid", bd=1).grid(row=1, column=0, sticky="nsew")
        tk.Label(subframe_rapida, text=d.get(f'g{group_index}_curva_rapida_tadic{suffix}'), font=FONTE_PADRAO, relief="solid", bd=1).grid(row=1, column=1, sticky="nsew")
...
class VisualizacaoFrame(tk.Frame):
    ...
    def _get_opcoes_formatadas(self, categoria):
        opcoes_raw = self.controller.get_opcoes_por_categoria(categoria)
    ...
    def criar_widgets(self):
        ...
        opcoes_malha = self._get_opcoes_formatadas('malha')
        tk.Label(frame_filtros, text="Malha:", font=FONTE_PADRAO).grid(row=0, column=2, padx=5, pady=5, sticky="w")
        ttk.Combobox(frame_filtros, textvariable=self.filtro_malha, values=[""] + opcoes_malha, state="readonly", font=FONTE_PADRAO, width=28).grid(row=0, column=3, padx=5, pady=5)

        tk.Label(frame_filtros, text="Alimentador:", font=FONTE_PADRAO).grid(row=1, column=0, padx=5, pady=5, sticky="w")
        opcoes_alimentador = self._get_opcoes_formatadas('alimentador')
        ttk.Combobox(frame_filtros, textvariable=self.filtro_alimentador, values=[""] + opcoes_alimentador, width=28, font=FONTE_PADRAO).grid(row=1, column=1, padx=5, pady=5)
        
        opcoes_condicao = self._get_opcoes_formatadas('condicao')
        tk.Label(frame_filtros, text="Condição:", font=FONTE_PADRAO).grid(row=1, column=2, padx=5, pady=5, sticky="w")
        ttk.Combobox(frame_filtros, textvariable=self.filtro_condicao, values=[""] + opcoes_condicao, state="readonly", font=FONTE_PADRAO, width=28).grid(row=1, column=3, padx=5, pady=5)
