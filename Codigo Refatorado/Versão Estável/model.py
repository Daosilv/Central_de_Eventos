...
class DatabaseModel:
    ...
    def get_categorias_gerais(self):
        ...
        categorias_fixas = ['bypass', 'malha', 'condicao', 'motivador', 'midia', 'telecontrolado', 'manobra_efetiva', 'vinculado_acessante', 'alimentador']
        return sorted(list(set(categorias_db + categorias_fixas)))

    def get_opcoes_por_categoria_geral(self, categoria):
        return self._execute("SELECT id, valor FROM opcoes_gerais WHERE categoria = ? ORDER BY valor", (categoria,), fetch='all')
    ...
    def get_opcoes_hierarquicas(self, id_pai=None):
        if id_pai is None:
            return self._execute("SELECT * FROM opcoes_hierarquicas WHERE id_pai IS NULL ORDER BY valor", fetch='all')
        else:
            return self._execute("SELECT * FROM opcoes_hierarquicas WHERE id_pai = ? ORDER BY valor", (id_pai,), fetch='all')
    ...
    def get_todos_cabos(self):
        return self._execute("SELECT * FROM cabos_propriedades ORDER BY trecho", fetch='all')
    ...
    def get_recent_equipments_with_status(self, limit=50):
        ...
        query = """
            SELECT
                c.eqpto, c.ns, c.data, c.alimentador,
