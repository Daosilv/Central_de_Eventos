...
# --- CLASSE PARA COMBOBOX COM AUTOCOMPLETE APRIMORADA ---
class AutocompleteCombobox(ttk.Combobox):
    ...
    def set_completion_list(self, completion_list):
        """Seta a lista completa de valores para o autocomplete."""
        self._completion_list = sorted(list(set(completion_list)))
        self['values'] = self._completion_list
    
    def _on_keyrelease(self, event):
        """Filtra a lista e mostra a lista suspensa ao digitar."""
    ...
    def _on_focus_out(self, event):
        """Garante que o valor digitado seja válido ou limpa o campo."""
        current_value = self.get()
        if current_value and self['values'] and current_value not in self['values']:
            # Força a seleção do primeiro item correspondente se houver correspondência parcial
            if self['values'][0].lower().startswith(current_value.lower()):
                self.set(self['values'][0])
            else:
                self.set('') # Limpa se não houver correspondência válida
...
class MemorialCalculoFrame(tk.Frame):
    ...
    def _criar_layout_cabecalho(self):
        ...
        self._criar_celula_cabecalho(row1_frame, "Condição:", self.campos_dict["Condição"], AutocompleteCombobox, {}, 8, 20, categoria_db='condicao', tipo_lista='geral')
        ...
        self._criar_celula_cabecalho(row4_frame, "By-Pass:", self.campos_dict["Bypass"], AutocompleteCombobox, {}, 6, 10, categoria_db='bypass', tipo_lista='geral')
    ...
    def _set_widgets_state(self, parent, new_state):
        for child in parent.winfo_children():
            if hasattr(child, 'config') and 'state' in child.config():
                if child.winfo_class() == 'TCombobox': child.config(state='normal' if new_state == 'normal' else 'disabled')
    ...
    def reconstruir_lista_ajustes(self, *args):
        ...
        for i, item in enumerate(filtrados):
            ...
            cor_led = 'grey'
            if status == 'Andamento': cor_led = '#f0ad4e' # Amarelo
            elif status == 'Aprovado': cor_led = '#5cb85c' # Verde
            elif status == 'Reprovado': cor_led = '#d9534f' # Vermelho
            
            # --- WIDGETS DA LINHA (AGORA EM UMA GRADE ÚNICA) ---
            ...
            btn_obs.grid(row=i, column=2, sticky='w', padx=2)

            tk.Label(self.ajustes_items_frame, text=item.get('alimentador', ''), width=8, anchor='w', bg='white', padx=5, font=FONTE_PEQUENA).grid(row=i, column=3, sticky='w')
            tk.Label(self.ajustes_items_frame, text=f"NS: {item.get('ns','')}", width=12, anchor='w', bg='white', padx=5, font=FONTE_PEQUENA).grid(row=i, column=4, sticky='w')
            tk.Label(self.ajustes_items_frame, text=item.get('data', ''), width=10, anchor='w', bg='white', padx=5, font=FONTE_PEQUENA).grid(row=i, column=5, sticky='w')
    ...
    def reconstruir_lista_rascunhos(self, *args):
        ...
        for i, item in enumerate(filtrados):
            ...
            lbl_data = tk.Label(self.rascunhos_items_frame, text=item.get('data', ''), width=10, anchor='w', bg='white', font=FONTE_PEQUENA)
            ...
            # Ponderador para a última coluna se expandir
