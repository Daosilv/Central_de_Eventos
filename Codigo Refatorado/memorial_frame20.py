...
class MemorialCalculoFrame(tk.Frame):
    ...
    def _criar_layout_cabecalho(self):
        ...
        self._criar_celula_cabecalho(row1_frame, "Autor:", self.campos_dict["Autor"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 4, 30)
        self._criar_celula_cabecalho(row1_frame, "Data:", self.campos_dict["Data"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 4, 10)
        self._criar_celula_cabecalho(row1_frame, "NS:", self.campos_dict["NS"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 3, 10)
        self._criar_celula_cabecalho(row1_frame, "Eqpto:", self.campos_dict["Eqpto"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 5, 10)
        ...
        self._criar_celula_cabecalho(row1_frame, "Condição:", self.campos_dict["Condição"], ttk.Combobox, {"values": opcoes_condicao, "state": "readonly", "font": FONTE_PADRAO, "style": 'Arrowless.TCombobox'}, 8, 20)
        ...
        self._criar_celula_cabecalho(row2_frame, "Malha:", self.campos_dict["Malha"], ttk.Combobox, {"values": opcoes_malha, "state": "readonly", "font": FONTE_PADRAO, "style": 'Arrowless.TCombobox'}, 6, 15)
        self._criar_celula_cabecalho(row2_frame, "Alimentador:", self.campos_dict["Alimentador"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 10, 15)
        opcoes_motivador = ["", "Instalação", "Reajuste", "Relocação"]
        self._criar_celula_cabecalho(row2_frame, "Motivador:", self.campos_dict["Motivador"], ttk.Combobox, {"values": opcoes_motivador, "state": "readonly", "font": FONTE_PADRAO, "style": 'Arrowless.TCombobox'}, 8, 15)
        self._criar_celula_cabecalho(row2_frame, "Coord:", self.campos_dict["Coord"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 5, 15)
        self._criar_celula_cabecalho(row2_frame, "Endereço:", self.campos_dict["Endereço"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 8, 30)

        # Linha 3
        row3_frame = tk.Frame(frame_campos, bg='white'); row3_frame.pack(fill="x", expand=True)
        opcoes_midia = ["", "Celular", "Satélite", "Rádio", "Desconhecido", "Não enc."]
        self._criar_celula_cabecalho(row3_frame, "Mídia:", self.campos_dict["Mídia"], ttk.Combobox, {"values": opcoes_midia, "state": "readonly", "font": FONTE_PADRAO, "style": 'Arrowless.TCombobox'}, 6, 15)
        ...
        self._criar_celula_cabecalho(row3_frame, "Fabricante:", self.campos_dict["Fabricante"], ttk.Combobox, {"values": opcoes_fabricante, "state": "readonly", "font": FONTE_PADRAO, "style": 'Arrowless.TCombobox'}, 10, 20)
        self._criar_celula_cabecalho(row3_frame, "Modelo:", self.campos_dict["Modelo"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 6, 20)
        self._criar_celula_cabecalho(row3_frame, "Comando:", self.campos_dict["Comando"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 8, 15)
        opcoes_tele = ["", "sim", "não"]
        self._criar_celula_cabecalho(row3_frame, "Telecontrolado:", self.campos_dict["Telecontrolado"], ttk.Combobox, {"values": opcoes_tele, "state": "readonly", "font": FONTE_PADRAO, "style": 'Arrowless.TCombobox'}, 12, 10)
        self._criar_celula_cabecalho(row3_frame, "Siom:", self.campos_dict["Siom"], tk.Entry, {"relief": "solid", "bd": 1, "font": FONTE_PADRAO}, 4, 10)

    def _criar_celula_cabecalho(self, parent, label_text, var, widget_class, widget_options, label_width, entry_width):
        ...
        if widget_class == ttk.Combobox:
            outer_entry = tk.Frame(frame, relief="solid", bd=1)
            widget = widget_class(outer_entry, textvariable=var, **widget_options)
            widget.pack()
            outer_entry.pack(side='left', fill='both', expand=True)
        else:
    ...
    def _criar_novas_tabelas_parametros(self, parent):
        ...
        for i, label in enumerate(labels_coord[:4]):
            ...
            tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_consumo[i]], font=FONTE_PADRAO, justify='center', relief='solid', bd=1, width=8).grid(row=i+2, column=1, sticky='nsew')
            tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_injecao[i]], font=FONTE_PADRAO, justify='center', relief='solid', bd=1, width=8).grid(row=i+2, column=2, sticky='nsew')
        ...
        tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_consumo[4]], font=FONTE_PADRAO, justify='center', relief='solid', bd=1).grid(row=6, column=1, columnspan=2, sticky='nsew')
        ...
        tk.Entry(coluna_esquerda, textvariable=self.campos_dict[vars_injecao[4]], font=FONTE_PADRAO, justify='center', relief='solid', bd=1).grid(row=7, column=1, columnspan=2, sticky='nsew')
        ...
        for i, label in enumerate(labels_montante):
            row_num = i + 10
            tk.Label(coluna_esquerda, text=label, font=FONTE_PADRAO, relief="solid", bd=1, anchor='w', width=20).grid(row=row_num, column=0, sticky='nsew')
            tk.Entry(coluna_esquerda, textvariable=self.campos_dict[f"{vars_montante[i]}_l1"], font=FONTE_PADRAO, justify='center', relief='solid', bd=1, width=8).grid(row=row_num, column=1, sticky='nsew')
            tk.Entry(coluna_esquerda, textvariable=self.campos_dict[f"{vars_montante[i]}_l2"], font=FONTE_PADRAO, justify='center', relief='solid', bd=1, width=8).grid(row=row_num, column=2, sticky='nsew')
