...
class DatabaseModel:
    ...
    def create_tables(self, columns_for_creation):
        ...
        # Verifica e atualiza a tabela de usuários se necessário
    ...
    # --- NOVA FUNÇÃO PARA ATUALIZAR A TABELA DE USUÁRIOS ---
    def _migrate_user_table(self):
        """Verifica as colunas da tabela de usuários e adiciona as que estiverem faltando."""
        try:
            # Pega as informações da tabela de usuários
            table_info = self._execute("PRAGMA table_info(usuarios)", fetch='all')
            if not table_info:
                return # Tabela não existe, será criada normalmente

            # Cria um conjunto com os nomes das colunas existentes
            existing_columns = {col['name'] for col in table_info}

            # Define as colunas que devem existir
            ...
            # Adiciona as colunas que estiverem faltando
            for col_name in missing_columns:
                print(f"Adicionando coluna '{col_name}' à tabela 'usuarios'...")
                self._execute(f"ALTER TABLE usuarios ADD COLUMN {col_name} TEXT", commit=True)
        except Exception as e:
            print(f"Erro ao migrar a tabela de usuários: {e}")
    # --- FIM DA NOVA FUNÇÃO ---
    ...
    def add_user(self, data):
        query = """
            INSERT INTO usuarios (nome_completo, matricula, email, permissoes, dados_adicionais)
            VALUES (:nome_completo, :matricula, :email, :permissoes, :dados_adicionais)
        """
