...
class AutocompleteCombobox(ttk.Combobox):
    ...
    def set_completion_list(self, completion_list):
        """Seta a lista completa de valores para o autocomplete, mantendo a ordem original."""
        self._completion_list = list(dict.fromkeys(completion_list))
...
class MemorialCalculoFrame(tk.Frame):
    ...
    def _criar_novas_tabelas_parametros(self, parent):
        ...
        for i, (label, var) in enumerate(params_tensao): tk.Label(f3, text=label, font=FONTE_PADRAO, bg='white', relief="solid", bd=1, width=18, anchor='w').grid(row=i+1, column=0, sticky="nsew"); tk.Entry(f3, textvariable=var, font=FONTE_PADRAO, justify="center", relief="solid", bd=1, width=8).grid(row=i+1, column=1, sticky="nsew")
    ...
    def reconstruir_lista_ajustes(self, *args):
        ...
        for i, item in enumerate(filtrados):
            ...
            eqpto_var = tk.StringVar(value=eqpto)
            entry_eqpto = tk.Entry(self.ajustes_items_frame, textvariable=eqpto_var, font=FONTE_PEQUENA, 
                                   state='readonly', relief='flat', readonlybackground='white', fg='black',
                                   width=8)
            entry_eqpto.grid(row=i, column=1, sticky='w', padx=2)
            ...
            alim_var = tk.StringVar(value=item.get('alimentador', ''))
            entry_alim = tk.Entry(self.ajustes_items_frame, textvariable=alim_var, font=FONTE_PEQUENA, 
                                  state='readonly', relief='flat', readonlybackground='white', fg='black',
                                  width=10)
            entry_alim.grid(row=i, column=3, sticky='w', padx=5)

            ns_var = tk.StringVar(value=f"NS: {item.get('ns','')}")
            entry_ns = tk.Entry(self.ajustes_items_frame, textvariable=ns_var, font=FONTE_PEQUENA,
                                state='readonly', relief='flat', readonlybackground='white', fg='black',
                                width=14)
            entry_ns.grid(row=i, column=4, sticky='w', padx=5)
            ...
            tk.Label(self.ajustes_items_frame, text=data_formatada, width=10, anchor='w', bg='white', padx=5, font=FONTE_PEQUENA).grid(row=i, column=5, sticky='w')

            for widget in [led, entry_eqpto]:
    ...
    def reconstruir_lista_rascunhos(self, *args):
        ...
        for i, item in enumerate(filtrados):
            ...
            # --- APLICANDO A MESMA LÓGICA DE TEXTO COPIÁVEL PARA RASCUNHOS ---
            eqpto_var = tk.StringVar(value=eqpto)
            entry_eqpto = tk.Entry(self.rascunhos_items_frame, textvariable=eqpto_var, font=FONTE_PEQUENA,
                                   state='readonly', relief='flat', readonlybackground='white', fg='black',
                                   width=8)
            entry_eqpto.grid(row=i, column=1, sticky='w', padx=(5, 0))

            alim_var = tk.StringVar(value=item.get('alimentador', ''))
            entry_alim = tk.Entry(self.rascunhos_items_frame, textvariable=alim_var, font=FONTE_PEQUENA,
                                  state='readonly', relief='flat', readonlybackground='white', fg='black',
                                  width=10)
            entry_alim.grid(row=i, column=2, sticky='w', padx=5)

            ns_var = tk.StringVar(value=f"NS: {item.get('ns', '')}")
            entry_ns = tk.Entry(self.rascunhos_items_frame, textvariable=ns_var, font=FONTE_PEQUENA,
                                state='readonly', relief='flat', readonlybackground='white', fg='black',
                                width=14)
            entry_ns.grid(row=i, column=3, sticky='w', padx=5)
            ...
            # Garante que o duplo clique para carregar o rascunho funcione em todos os campos
            for widget in [entry_eqpto, entry_alim, entry_ns, lbl_data]:
    ...
    def preencher_formulario(self, dados_dict):
        ...
        self.text_obs.insert("1.0", dados_dict.get("observacoes") or "")
