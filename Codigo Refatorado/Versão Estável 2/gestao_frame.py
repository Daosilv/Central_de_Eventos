...
class MultiAddDialog(tk.Toplevel):
    """Janela para adicionar múltiplos itens de uma só vez (um por linha)."""
...
class MultiCaboDialog(tk.Toplevel):
    """Janela específica para adicionar múltiplos cabos no formato CSV."""
    def __init__(self, parent, title):
        super().__init__(parent)
        self.result = None
        self.title(title)
...
class TabListasGerais(tk.Frame):
    ...
    def adicionar_opcao(self):
        """
        Adiciona uma ou mais opções à categoria selecionada.
        """
        categoria = self.combo_categorias.get() #
        if not categoria:
        ...
        # Abre a janela de diálogo para múltiplos itens
        dialog = MultiAddDialog(self, f"Adicionar Itens para '{categoria}'") #
        
        # Se o usuário confirmou e há resultados
        if dialog.result: #
            count = 0
...
class TabHierarquia(tk.Frame):
    ...
    def adicionar_multiplos_itens(self):
        ...
        if dialog.result:
            ...
            self.carregar_hierarquia()
            self.tree.selection_set(selected_id) # Mantém a seleção
