...
class MemorialCalculoFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # --- Variáveis e Dicionários de Controle ---
    ...
    def criar_layout_principal(self):
        scroll_container = tk.Frame(self, bg='white'); scroll_container.pack(fill="both", expand=True)
        ...
        self.content_holder_frame.grid_columnconfigure(0, minsize=900); self.content_holder_frame.grid_columnconfigure(1, weight=0); self.content_holder_frame.grid_rowconfigure(1, weight=1)
        self.header_frame = tk.Frame(self.content_holder_frame, bg='white'); self.header_frame.grid(row=0, column=0, sticky="new", padx=10, pady=10)
        self.main_form_frame = tk.Frame(self.content_holder_frame, bg='white'); self.main_form_frame.grid(row=1, column=0, sticky="nsew", padx=10)
        sidebar_frame = tk.LabelFrame(self.content_holder_frame, text="Histórico", width=SIDEBAR_WIDTH, padx=5, pady=5, bg='white'); sidebar_frame.grid(row=0, column=1, rowspan=2, sticky="ns", pady=10)
        ...
        ajustes_canvas_frame = tk.Frame(ajustes_container, bg='white', relief="solid", bd=1); ajustes_canvas_frame.pack(fill='both', expand=True, pady=(5,0)); self.ajustes_canvas = tk.Canvas(ajustes_canvas_frame, highlightthickness=0, bg='white'); ajustes_scrollbar = ttk.Scrollbar(ajustes_canvas_frame, orient="vertical", command=self.ajustes_canvas.yview); self.ajustes_items_frame = tk.Frame(self.ajustes_canvas, bg='white'); self.ajustes_window_id = self.ajustes_canvas.create_window((0, 0), window=self.ajustes_items_frame, anchor="nw"); self.ajustes_canvas.bind("<Configure>", self.on_ajustes_canvas_configure); self.ajustes_canvas.configure(yscrollcommand=ajustes_scrollbar.set); ajustes_scrollbar.pack(side="right", fill="y"); self.ajustes_canvas.pack(side="left", fill="both", expand=True); self.ajustes_items_frame.bind("<Configure>", lambda e: self.ajustes_canvas.configure(scrollregion=self.ajustes_canvas.bbox("all")))
        ...
        rascunhos_canvas_frame = tk.Frame(rascunhos_container, bg='white', relief="solid", bd=1); rascunhos_canvas_frame.pack(fill='both', expand=True, pady=5); self.rascunhos_canvas = tk.Canvas(rascunhos_canvas_frame, highlightthickness=0, bg='white'); rascunhos_scrollbar = ttk.Scrollbar(rascunhos_canvas_frame, orient="vertical", command=self.rascunhos_canvas.yview); self.rascunhos_items_frame = tk.Frame(self.rascunhos_canvas, bg='white'); self.rascunhos_window_id = self.rascunhos_canvas.create_window((0, 0), window=self.rascunhos_items_frame, anchor="nw"); self.rascunhos_canvas.bind("<Configure>", self.on_rascunhos_canvas_configure); self.rascunhos_canvas.configure(yscrollcommand=rascunhos_scrollbar.set); rascunhos_scrollbar.pack(side="right", fill="y"); self.rascunhos_canvas.pack(side="left", fill="both", expand=True); self.rascunhos_items_frame.bind("<Configure>", lambda e: self.rascunhos_canvas.configure(scrollregion=self.canvas.bbox("all")))
    ...
    def _criar_layout_cabecalho(self):
        ...
        self._criar_celula_cabecalho(row1_frame, "Condição:", self.campos_dict["Condição"], ttk.Combobox, {}, 8, 20, categoria_db='condicao', tipo_lista='geral')

        # Linha 2
        row2_frame = tk.Frame(frame_campos, bg='white'); row2_frame.pack(fill="x", expand=True, pady=(0, 2))
        self._criar_celula_cabecalho(row2_frame, "Malha:", self.campos_dict["Malha"], ttk.Combobox, {}, 6, 10, categoria_db='malha', tipo_lista='geral')
        self._criar_celula_cabecalho(row2_frame, "Alimentador:", self.campos_dict["Alimentador"], ttk.Combobox, {}, 10, 17, categoria_db='alimentador', tipo_lista='geral')
        self._criar_celula_cabecalho(row2_frame, "Motivador:", self.campos_dict["Motivador"], ttk.Combobox, {}, 8, 8, categoria_db='motivador', tipo_lista='geral')
        ...
        self._criar_celula_cabecalho(row3_frame, "Mídia:", self.campos_dict["Mídia"], ttk.Combobox, {}, 6, 10, categoria_db='midia', tipo_lista='geral')
        # --- HIERARQUIA ---
        self._criar_celula_cabecalho(row3_frame, "Fabricante:", self.campos_dict["Fabricante"], ttk.Combobox, {}, 10, 20, nome_widget='fabricante')
        self._criar_celula_cabecalho(row3_frame, "Modelo:", self.campos_dict["Modelo"], ttk.Combobox, {}, 6, 20, nome_widget='modelo')
        self._criar_celula_cabecalho(row3_frame, "Comando:", self.campos_dict["Comando"], ttk.Combobox, {}, 8, 15, nome_widget='comando')
        # --- FIM HIERARQUIA ---
        self._criar_celula_cabecalho(row3_frame, "Telecontrolado:", self.campos_dict["Telecontrolado"], ttk.Combobox, {}, 12, 5, categoria_db='telecontrolado', tipo_lista='geral')
        ...
        self._criar_celula_cabecalho(row4_frame, "Manobra Efetiva:", self.campos_dict["Manobra Efetiva"], ttk.Combobox, {}, 14, 5, categoria_db='manobra_efetiva', tipo_lista='geral')
        self._criar_celula_cabecalho(row4_frame, "Vinculado a Acessante:", self.campos_dict["Vinculado a Acessante"], ttk.Combobox, {}, 19, 5, categoria_db='vinculado_acessante', tipo_lista='geral')
    ...
    def _criar_celula_cabecalho(self, parent, label_text, var, widget_class, widget_options, label_width, entry_width, validate_cmd=None, categoria_db=None, tipo_lista=None, nome_widget=None):
        ...
        if widget_class == ttk.Combobox:
            if tipo_lista == 'geral' and categoria_db:
                options['values'] = [""] + self._get_opcoes_formatadas_geral(categoria_db)
            ...
            elif categoria_db: # Para as listas gerais
                self.comboboxes_cabecalho[categoria_db] = widget
        else: 
            if validate_cmd: options['validate'] = 'key'; options['validatecommand'] = validate_cmd
            widget = widget_class(widget_container, **options)
    ...
    def atualizar_todas_as_listas(self):
        ...
        # Carrega fabricantes (nível superior da hierarquia)
    ...
    def _carregar_fabricantes(self):
        ...
        combo['values'] = [""] + sorted(self.fabricantes_map.keys())
    ...
    def _carregar_cabos(self):
        ...
        combo['values'] = [""] + sorted(self.cabos_map.keys())
    ...
    def _on_fabricante_select(self, event=None):
        ...
        self.comboboxes_cabecalho['modelo']['values'] = [""]
        self.comboboxes_cabecalho['comando']['values'] = [""]
        ...
        combo_modelo['values'] = [""] + sorted(self.modelos_map.keys())
    ...
    def _on_modelo_select(self, event=None):
        ...
        combo_comando['values'] = [""] + sorted(self.comandos_map.keys())
    ...
    def criar_layout_formulario(self):
        ...
        botoes_frame = tk.Frame(self.main_form_frame, bg='white'); botoes_frame.pack(pady=20); ttk.Button(botoes_frame, text="Buscar", command=self.abrir_janela_busca).pack(side="left", padx=5); ttk.Button(botoes_frame, text="Salvar Rascunho", command=self.salvar_rascunho).pack(side="left", padx=5); ttk.Button(botoes_frame, text="Salvar e Enviar", command=self.salvar_e_enviar).pack(side="left", padx=5); ttk.Button(botoes_frame, text="Limpar", command=self.limpar_formulario).pack(side="left", padx=5)
    ...
    def _criar_novas_tabelas_parametros(self, parent):
        ...
        f3 = tk.Frame(coluna_meio, bg='white'); f3.pack(anchor='w', pady=(0, 10)); params_tensao = [("Tensão [kV] da rede", self.campos_dict["tensao_kv_rede"]), ("Carga - fase A [A]", self.campos_dict["carga_fase_a"]), ("Carga - fase B [A]", self.campos_dict["carga_fase_b"]), ("Carga - fase C [A]", self.campos_dict["carga_fase_c"])]
        for i, (label, var) in enumerate(params_tensao): tk.Label(f3, text=label, font=FONTE_PADRAO, bg='white', relief="solid", bd=1, width=18, anchor='w').grid(row=i, column=0, sticky="nsew"); tk.Entry(f3, textvariable=var, font=FONTE_PADRAO, justify="center", relief="solid", bd=1, width=8).grid(row=i, column=1, sticky="nsew")
        ...
        combo_cabo = ttk.Combobox(f4, textvariable=self.campos_dict["cabo_critico_trecho"], font=FONTE_PADRAO, justify="center", width=14, state='readonly')
    ...
    def on_ajustes_canvas_configure(self, event): self.ajustes_canvas.itemconfig(self.ajustes_window_id, width=event.width)
    def on_rascunhos_canvas_configure(self, event): self.rascunhos_canvas.itemconfig(self.rascunhos_window_id, width=event.width)
    def _gerenciar_campo_acessante(self, *args):
        if self.campos_dict["Vinculado a Acessante"].get() == "Sim": self.frame_acessante.pack(side='left', fill='y', padx=(0,5))
        else: self.frame_acessante.pack_forget(); self.campos_dict["Eqpto Acessante"].set("")
    def _construir_bloco_tabelas(self, parent, vars_storage, tables_storage, refs_storage, tipo_bloco):
        header = tk.Frame(parent, bd=1, relief="solid", bg='white'); [header.grid_columnconfigure(c, minsize=w) for c, w in enumerate(self.col_widths)]; [header.grid_rowconfigure(r, minsize=30, weight=1) for r in range(3)]; self._cell(header, 0, 0, "Grupo Normal = Grupo de Consumo", cs=9); self._cell(header, 1, 0, "  Grupos  ", rs=2); self._cell(header, 1, 1, "FASE", cs=4); self._cell(header, 1, 5, "TERRA", cs=4); self._cell(header, 2, 1, "  Pickup  "); self._cell(header, 2, 2, "  Sequência  "); self._cell(header, 2, 3, "  Curva Lenta  "); self._cell(header, 2, 4, "  Curva Rápida  "); self._cell(header, 2, 5, "  Pickup  "); self._cell(header, 2, 6, "  Sequência  "); self._cell(header, 2, 7, "  Curva Lenta  "); self._cell(header, 2, 8, "  Curva Rápida  "); header.pack(pady=5)
    ...
    def _gerenciar_bloco_tabelas_replicado(self, *args):
        ...
        if new_state == 'disabled': [var.set("") for g in self.vars_grupos_rep.values() for var in g.values()]
    def _alinhar_coluna_critica_replicada(self):
        self.update_idletasks()
        if not self.ref_cells_rep or not self.ref_cells_rep[0].winfo_exists(): return
        ref_w = self.ref_cells_rep[0].winfo_width()
        if ref_w > 1:
            if self.frame_tabela_header_rep: self.frame_tabela_header_rep.grid_columnconfigure(3, minsize=ref_w)
            for table in self.tabelas_grupo_frames_rep: table.grid_columnconfigure(3, minsize=ref_w)
    ...
    def alinhar_coluna_critica(self):
        ...
        ref_w = self.ref_cells[0].winfo_width()
        if ref_w > 1:
            if self.frame_tabela_header: self.frame_tabela_header.grid_columnconfigure(3, minsize=ref_w)
            for table in self.tabelas_grupo_frames: table.grid_columnconfigure(3, minsize=ref_w)
    ...
    def open_dropdown_on_click(self, event): event.widget.event_generate('<Down>')
    ...
    def criar_tabela_grupo(self, parent, group_name):
        ...
        group_vars["sequencia"].trace_add("write", _sync_seq); group_vars["sequencia"].trace_add("write", _manage_rapida); tk.Label(frame_tabela, text=group_name, relief="solid", bd=1, font=FONTE_PADRAO, bg='white').grid(row=1, column=0, rowspan=3, sticky="nsew"); tk.Label(frame_tabela, text="FASE", relief="solid", bd=1, font=FONTE_PADRAO, bg='white').grid(row=1, column=1, columnspan=2, sticky="nsew"); lenta_fase_cell = tk.Frame(frame_tabela, relief="solid", bd=1, bg='white'); combo_lenta = ttk.Combobox(lenta_fase_cell, textvariable=group_vars["curva_lenta_tipo"], values=["", "IEC Muito Inversa", "IEC Ext. Inversa"], state="readonly", font=FONTE_PEQUENA, justify="center", style='Arrowless.TCombobox'); combo_lenta.bind("<Button-1>", self.open_dropdown_on_click); combo_lenta.pack(fill='both', expand=True); lenta_fase_cell.grid(row=1, column=3, sticky="nsew"); self._cell(frame_tabela, 1, 4, "IEC Inversa"); tk.Label(frame_tabela, text="TERRA", relief="solid", bd=1, font=FONTE_PADRAO, bg='white').grid(row=1, column=5, columnspan=2, sticky="nsew"); tk.Label(frame_tabela, text="T. Definido", relief="solid", bd=1, font=FONTE_PADRAO, bg='white').grid(row=1, column=7, sticky="nsew"); tk.Label(frame_tabela, text="T. Definido", relief="solid", bd=1, font=FONTE_PADRAO, bg='white').grid(row=1, column=8, sticky="nsew"); tk.Entry(frame_tabela, textvariable=group_vars["pickup_fase"], font=FONTE_PADRAO, width=4, relief="solid", bd=1, justify="center", validate="key", validatecommand=(self.vcmd_4, "%P")).grid(row=1, column=1, rowspan=3, sticky="nsew"); cell_seq = tk.Frame(frame_tabela, relief="solid", bd=1, bg='white'); cell_seq.grid_propagate(False); cell_seq.grid(row=1, column=2, rowspan=3, sticky="nsew"); combo_seq = ttk.Combobox(cell_seq, textvariable=group_vars["sequencia"], values=["", "1L", "2L", "3L", "1R+3L", "1R+2L", "2R+2L"], state="readonly", font=FONTE_PADRAO, justify="center", width=8, style='Arrowless.TCombobox'); combo_seq.bind("<Button-1>", self.open_dropdown_on_click); combo_seq.pack(fill="both", expand=True); tk.Entry(frame_tabela, textvariable=group_vars["pickup_terra"], font=FONTE_PADRAO, width=4, relief="solid", bd=1, justify="center", validate="key", validatecommand=(self.vcmd_4, "%P")).grid(row=1, column=5, rowspan=3, sticky="nsew"); cell_seq_terra = tk.Frame(frame_tabela, relief="solid", bd=1, bg='white'); cell_seq_terra.grid_propagate(False); cell_seq_terra.grid(row=1, column=6, rowspan=3, sticky="nsew"); tk.Label(cell_seq_terra, textvariable=group_vars["sequencia_terra"], font=FONTE_PADRAO, justify="center", bg='white').pack(fill="both", expand=True); sub_lenta = tk.Frame(frame_tabela, bd=0, bg='white'); sub_lenta.grid_propagate(False); sub_lenta.grid(row=2, column=3, rowspan=2, sticky="nsew"); [sub_lenta.grid_rowconfigure(r, weight=1) for r in range(2)]; [sub_lenta.grid_columnconfigure(c, weight=1) for c in range(2)]; tk.Label(sub_lenta, text="Dial", font=FONTE_PADRAO, width=8, relief="solid", bd=1, bg='white').grid(row=0, column=0, sticky="nsew"); tk.Label(sub_lenta, text="T. Adic.", font=FONTE_PADRAO, width=8, relief="solid", bd=1, bg='white').grid(row=0, column=1, sticky="nsew"); tk.Entry(sub_lenta, textvariable=group_vars["curva_lenta_dial"], font=FONTE_PADRAO, width=4, relief="solid", bd=1, justify="center", validate="key", validatecommand=(self.vcmd_4, "%P")).grid(row=1, column=0, sticky="nsew"); tk.Entry(sub_lenta, textvariable=group_vars["curva_lenta_tadic"], font=FONTE_PADRAO, width=4, relief="solid", bd=1, justify="center", validate="key", validatecommand=(self.vcmd_4, "%P")).grid(row=1, column=1, sticky="nsew"); sub_rapida = tk.Frame(frame_tabela, bd=0, bg='white'); sub_rapida.grid_propagate(False); sub_rapida.grid(row=2, column=4, rowspan=2, sticky="nsew"); [sub_rapida.grid_rowconfigure(r, weight=1) for r in range(2)]; [sub_rapida.grid_columnconfigure(c, weight=1) for c in range(2)]; tk.Label(sub_rapida, text="Dial", font=FONTE_PADRAO, width=8, relief="solid", bd=1, bg='white').grid(row=0, column=0, sticky="nsew"); tk.Label(sub_rapida, text="T. Adic.", font=FONTE_PADRAO, width=8, relief="solid", bd=1, bg='white').grid(row=0, column=1, sticky="nsew"); entry_fase_rapida_dial = tk.Entry(sub_rapida, textvariable=group_vars["curva_rapida_dial"], font=FONTE_PADRAO, width=4, relief="solid", bd=1, justify="center", validate="key", validatecommand=(self.vcmd_4, "%P")); entry_fase_rapida_dial.grid(row=1, column=0, sticky="nsew"); entry_fase_rapida_tadic = tk.Entry(sub_rapida, textvariable=group_vars["curva_rapida_tadic"], font=FONTE_PADRAO, width=4, relief="solid", bd=1, justify="center", validate="key", validatecommand=(self.vcmd_4, "%P")); entry_fase_rapida_tadic.grid(row=1, column=1, sticky="nsew"); self._cell(frame_tabela, 2, 7, "Tempo (s)"); self._cell(frame_tabela, 2, 8, "Tempo (s)"); tk.Entry(frame_tabela, textvariable=group_vars["terra_tempo_lenta"], font=FONTE_PADRAO, width=4, relief="solid", bd=1, justify="center", validate="key", validatecommand=(self.vcmd_4, "%P")).grid(row=3, column=7, sticky="nsew"); entry_terra_rapida_tempo = tk.Entry(frame_tabela, textvariable=group_vars["terra_tempo_rapida"], font=FONTE_PADRAO, width=4, relief="solid", bd=1, justify="center", validate="key", validatecommand=(self.vcmd_4, "%P")); entry_terra_rapida_tempo.grid(row=3, column=8, sticky="nsew"); frame_tabela.pack(pady=(0, 5)); return frame_tabela, lenta_fase_cell, group_vars
    def _get_form_data(self):
        ...
        if self.campos_dict["Condição"].get() in ["Acess Inv", "Acess s/Inv"]:
            for i in range(1, 4):
    ...
    def reconstruir_lista_ajustes(self, *args):
        [w.destroy() for w in self.ajustes_items_frame.winfo_children()]; termo = self.busca_ajustes_var.get().lower()
        filtrados = [a for a in self.lista_ajustes_completa if a['eqpto'] and termo in a['eqpto'].lower()]
        for item in filtrados:
            row = tk.Frame(self.ajustes_items_frame, bg='white'); row.pack(fill='x', expand=True, pady=1)
            lbls = [tk.Label(row, text=item.get(k, '') or '', width=w, anchor='w', bg='white', padx=p, font=FONTE_PEQUENA) for k, w, p in [('eqpto',10,4), ('alimentador',10,5), (f"NS: {item.get('ns','')}",20,5), ('data',10,5)]]
            for i, lbl in enumerate(lbls): lbl.grid(row=0, column=i, sticky='w')
            [w.bind("<Double-1>", lambda e, eq=item['eqpto']: self.carregar_ajuste_pelo_nome(eq)) for w in [row]+lbls]
        self.ajustes_canvas.yview_moveto(0)
    def reconstruir_lista_rascunhos(self, *args):
        [w.destroy() for w in self.rascunhos_items_frame.winfo_children()]; self.rascunho_check_vars.clear(); termo = self.busca_rascunhos_var.get().lower()
        filtrados = [r for r in self.lista_rascunhos_completa if r['eqpto'] and termo in r['eqpto'].lower()]
        for item in filtrados:
            var = tk.BooleanVar(); self.rascunho_check_vars[item['eqpto']] = var; row = tk.Frame(self.rascunhos_items_frame, bg='white'); row.pack(fill='x', expand=True, pady=1)
            ttk.Checkbutton(row, variable=var).grid(row=0, column=0, sticky='w')
            lbls = [tk.Label(row, text=item.get(k, '') or '', width=w, anchor='w', bg='white', font=FONTE_PEQUENA) for k, w in [('eqpto',10), ('alimentador',10), (f"NS: {item.get('ns','')}",20), ('data',None)]]
            for i, lbl in enumerate(lbls): lbl.grid(row=0, column=i+1, sticky='w', padx=5 if i!=0 else (5,0))
            row.grid_columnconfigure(5, weight=1); tk.Button(row, text="X", fg="red", relief="flat", cursor="hand2", font=("Helvetica", 8, "bold"), command=lambda e=item['eqpto']: self.deletar_rascunho_individual(e), bg='white').grid(row=0, column=6, sticky='e', padx=(0,5))
            [w.bind("<Double-1>", lambda e, eq=item['eqpto']: self.carregar_rascunho_pelo_nome(eq)) for w in lbls]
        self.selecionar_tudo_rascunhos_var.set(False); self.rascunhos_canvas.yview_moveto(0)
    ...
    def preencher_formulario(self, dados_dict):
        ...
        for nome, var in self.campos_dict.items(): var.set(dados_dict.get(nome.lower().replace(" ", "_"), ""))
        for i in range(1, 4):
            if f'grupo_{i}' in self.vars_grupos: [v.set(dados_dict.get(f"g{i}_{k}", "")) for k, v in self.vars_grupos[f'grupo_{i}'].items()]
        for i in range(1, 4):
            if f'grupo_{i}' in self.vars_grupos_rep: [v.set(dados_dict.get(f"g{i}_{k}_rep", "")) for k, v in self.vars_grupos_rep[f'grupo_{i}'].items()]
        self.text_obs.insert("1.0", dados_dict.get("observacoes", "")); anexos_json = dados_dict.get("anexos", "[]")
        try: self.anexos_list = json.loads(anexos_json if anexos_json else "[]")
        except json.JSONDecodeError: self.anexos_list = []
    def abrir_janela_busca(self): messagebox.showinfo("Info", "Funcionalidade em desenvolvimento.")
