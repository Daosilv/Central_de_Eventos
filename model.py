...
class DatabaseModel:
    ...
    def _execute(self, query, params=(), commit=False, fetch=None, executemany=False):
        try:
            ...
            # Exibe o erro para o usuário final em vez de apenas no console
            print(f"Erro no banco de dados: {e}")
            traceback.print_exc()
            messagebox.showerror("Erro de Banco de Dados", f"Ocorreu um erro de banco de dados:\n\n{e}\n\nVerifique as permissões da pasta C:\\BD_APP_Cemig e do arquivo de banco de dados.")
            return None
    ...
    def populate_initial_data(self):
        """Popula o banco com dados iniciais para as listas suspensas, se estiverem vazias."""
        opcoes = {
            'condicao': ["Normal", "Acessante", "Acessante s/ Religador", "Contingência", "Ampliação", "Melhoria"],
            'motivador': ["Contingência", "Melhoria", "Ampliação", "Cliente Novo"],
            'midia': ["Rádio", "Fibra", "Satélite", "Celular"],
            'telecontrolado': ["Sim", "Não"],
            'bypass': ["Sim", "Não"],
            'manobra_efetiva': ["Sim", "Não"],
            'vinculado_acessante': ["Sim", "Não"],
            'tensao_kv_rede': ["13.8", "22.0", "34.5"]
        }

        for categoria, valores in opcoes.items():
            params = [(categoria, valor) for valor in valores]
            self._execute("INSERT OR IGNORE INTO opcoes_gerais (categoria, valor) VALUES (?, ?)", params, commit=True, executemany=True)

        # Bloco de código de fabricantes fictícios foi comentado para não ser executado.
        # fabricantes = {
        #     "Schneider": {
        #         "U25": ["ADVC"],
        #         "RL27": ["FTU-R200"]
        ...
        # }
        #
        # for fab, modelos in fabricantes.items():
        #     self._execute("INSERT OR IGNORE INTO opcoes_hierarquicas (categoria, valor, id_pai) VALUES (?, ?, ?)", ("fabricante", fab, None), commit=True)
        #     fab_id_row = self._execute("SELECT id FROM opcoes_hierarquicas WHERE categoria = 'fabricante' AND valor = ?", (fab,), fetch='one')
        #     if fab_id_row:
        #         fab_id = fab_id_row['id']
        #         for mod, comandos in modelos.items():
        #             self._execute("INSERT OR IGNORE INTO opcoes_hierarquicas (categoria, valor, id_pai) VALUES (?, ?, ?)", ("modelo", mod, fab_id), commit=True)
        #             mod_id_row = self._execute("SELECT id FROM opcoes_hierarquicas WHERE categoria = 'modelo' AND valor = ? AND id_pai = ?", (mod, fab_id), fetch='one')
        #             if mod_id_row:
        #                 mod_id = mod_id_row['id']
        #                 for com in comandos:
        #                     self._execute("INSERT OR IGNORE INTO opcoes_hierarquicas (categoria, valor, id_pai) VALUES (?, ?, ?)", ("comando", com, mod_id), commit=True)
