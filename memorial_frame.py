...
class AutocompleteCombobox(ttk.Entry):
    def __init__(self, *args, **kwargs):
        ...
        # Cria popup sem bordas
        self.popup = tk.Toplevel(self)
        self.popup.wm_overrideredirect(True)
        self.popup.withdraw()

        # Listbox de sugestões
        self.listbox = tk.Listbox(self.popup, exportselection=False, font=FONTE_PEQUENA)
        ...
        self.listbox.bind("<Up>", self._handle_listbox_up)
        self.listbox.bind("<Escape>", lambda e: self._hide_popup())

        # Estado do popup e flag de seleção
        self._popup_open = False
        self._just_selected = False

    def set_completion_list(self, completion_list):
        """Define as opções para autocompletar."""
        self.completion_list = sorted(set(completion_list))
    ...
    def _update_listbox(self):
        val = self.get().lower()
        data = self.completion_list if not val else [
            item for item in self.completion_list
            if item.lower().startswith(val)
        ]
    ...
    def _on_select(self, event):
        ...
        if sel:
            value = self.listbox.get(sel)
            self.delete(0, tk.END)
            self.insert(0, value)
            # Previne filtragem imediata após seleção
            self._just_selected = True
        # Limpa a lista e fecha popup
        self.listbox.delete(0, tk.END)
        self._hide_popup()

    def _focus_listbox(self, event):
        if self._popup_open and self.listbox.size() > 0:
            self.listbox.focus_set()
            self.listbox.selection_set(0)

    def _handle_listbox_up(self, event):
        if self.listbox.curselection() and self.listbox.curselection()[0] == 0:
            self.focus_set()
...
class MemorialCalculoFrame(tk.Frame):
    ...
    def criar_layout_principal(self, scroll_container):
        ...
        sidebar_frame.pack(padx=2, pady=2, fill='both', expand=True)
        ...
        self.ajustes_canvas.create_window((0, 0), window=self.ajustes_items_frame, anchor="nw")
    ...
    def _criar_celula_cabecalho(self, parent, label_text, var, widget_class, widget_options, label_width, entry_width, categoria_db=None, tipo_lista=None, nome_widget=None, expand=False):
        ...
        if widget_class == AutocompleteCombobox:
        ...
        elif issubclass(widget_class, ttk.Combobox):
            widget = widget_class(widget_container, **options)
            if tipo_lista == 'geral' and categoria_db:
                opcoes = [""] + self._get_opcoes_formatadas_geral(categoria_db)
    ...
    def atualizar_todas_as_listas(self):
        ...
        for categoria, combobox in self.comboboxes_cabecalho.items():
            if categoria not in ['fabricante', 'modelo', 'comando', 'cabo_critico_trecho']:
                ...
                # A lógica de população precisa ser consistente com o tipo de widget
                if isinstance(combobox, AutocompleteCombobox):
                    combobox.set_completion_list(opcoes)
                else: # É um ttk.Combobox padrão
    ...
    def _carregar_fabricantes(self):
        ...
        # O bind para seleção agora é tratado dentro do próprio widget AutocompleteCombobox
        # combo.bind("<<ComboboxSelected>>", self._on_fabricante_select)
        # Em vez disso, usamos o trace da variável
        self.campos_dict["Fabricante"].trace_add("write", self._on_fabricante_select)
    ...
    def _carregar_cabos(self):
        ...
        self.campos_dict["cabo_critico_trecho"].trace_add("write", self._on_cabo_select)

    def _on_fabricante_select(self, *args):
        # Para evitar chamadas recursivas ou duplas, limpamos o modelo apenas se o fabricante mudou para um valor válido
        fab_selecionado = self.campos_dict["Fabricante"].get()
        
        # Se o campo de modelo já estiver vazio, não há necessidade de limpar de novo
        if self.campos_dict["Modelo"].get() != "":
            self.campos_dict["Modelo"].set("")
        else:
            # Se o modelo já está vazio, pode ser que o usuário esteja selecionando um item da lista,
            # então não queremos interferir, a menos que o fabricante esteja sendo zerado.
            if not fab_selecionado:
                 self.campos_dict["Modelo"].set("")
        ...
        self.campos_dict["Modelo"].trace_add("write", self._on_modelo_select)

    def _on_modelo_select(self, *args):
        mod_selecionado = self.campos_dict["Modelo"].get()
        if self.campos_dict["Comando"].get() != "":
            self.campos_dict["Comando"].set("")
    ...
    def _criar_celula_lateral(self, parent, height, widget_list, is_first_cell=False, is_group_cell=False):
        ...
        cell_frame = tk.Frame(outer_frame, bd=1, relief="solid", bg='white')
        ...
        if is_first_cell:
            ...
            text_widget.pack(fill='both', expand=True, padx=(0, 2), pady=2)
