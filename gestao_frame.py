...
class GestaoFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tab6 = TabAjuda(notebook, controller) 
...
class TabListasGerais(tk.Frame):
    ...
    def _criar_frame_listas_simples(self):
        ...
        ttk.Button(botoes_frame, text="Exportar para Excel", command=self.exportar_lista_simples_para_excel).pack(pady=5, fill='x')
    ...
    # --- NOVO MÉTODO DE EXPORTAÇÃO PARA LISTAS SIMPLES ---
    def exportar_lista_simples_para_excel(self):
        categoria = self.combo_categorias.get()
        if not categoria or categoria in ["Tempo Morto", "Grupo 4"]:
            messagebox.showwarning("Ação Inválida", "Selecione uma categoria de lista simples para exportar.", parent=self)
            return

        items = self.listbox_opcoes.get(0, tk.END)
        if not items:
            messagebox.showinfo("Lista Vazia", f"A categoria '{categoria}' não possui itens para exportar.", parent=self)
            return

        filepath = filedialog.asksaveasfilename(
            defaultextension=".xlsx",
            filetypes=[("Excel Workbook", "*.xlsx")],
            title=f"Salvar Lista '{categoria}' Como...",
            initialfile=f"Lista_{categoria}.xlsx"
        )
        if not filepath:
            return
        ...
        try:
            ...
            ws = wb.active
            ws.title = categoria[:31] # Limita o nome da aba para 31 caracteres
            ws.append([f"Itens da Categoria: {categoria}"]) # Cabeçalho
            for item in items:
                ws.append([item])
            
            wb.save(filepath)
            messagebox.showinfo("Sucesso", f"Lista exportada com sucesso para:\n{filepath}", parent=self)
        except Exception as e:
...
class TabHierarquia(tk.Frame):
    ...
    def criar_widgets(self):
        ...
        # NOVO BOTÃO DE EXPORTAÇÃO
        ttk.Button(botoes_frame, text="Exportar para Excel", command=self.exportar_hierarquia_para_excel).pack(pady=(15, 5), fill='x')
    ...
    # --- NOVO MÉTODO DE EXPORTAÇÃO PARA HIERARQUIA ---
    def exportar_hierarquia_para_excel(self):
        filepath = filedialog.asksaveasfilename(
            defaultextension=".xlsx",
            filetypes=[("Excel Workbook", "*.xlsx")],
            title="Salvar Hierarquia Como...",
            initialfile="Hierarquia_Fabricantes.xlsx"
        )
        if not filepath:
            return
        ...
        try:
            ...
            ws = wb.active
            ws.title = "Hierarquia"
            ws.append(['Fabricante', 'Modelo', 'Comando'])

            for fab_id in self.tree.get_children(''):
                fab_nome = self.tree.item(fab_id, 'text')
                modelos = self.tree.get_children(fab_id)
                if not modelos:
                    ws.append([fab_nome, '', ''])
                else:
                    for mod_id in modelos:
                        mod_nome = self.tree.item(mod_id, 'text')
                        comandos = self.tree.get_children(mod_id)
                        if not comandos:
                            ws.append([fab_nome, mod_nome, ''])
                        else:
                            for com_id in comandos:
                                com_nome = self.tree.item(com_id, 'text')
                                ws.append([fab_nome, mod_nome, com_nome])

            wb.save(filepath)
            messagebox.showinfo("Sucesso", f"Hierarquia exportada com sucesso para:\n{filepath}", parent=self)
        except Exception as e:
            messagebox.showerror("Erro na Exportação", f"Ocorreu um erro ao salvar o arquivo:\n{e}", parent=self)
...
# --- ABAS EXISTENTES (sem alterações, exceto as já modificadas acima) ---
